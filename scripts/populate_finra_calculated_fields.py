...
def main():
    """Main entry point."""
    _logger.info("FINRA Calculated Fields Populator starting")

    populator = FINRAFieldPopulator()
    results = populator.run_full_population()

    if 'error' in results:
        _logger.error("Population failed: %s", results['error'])
        return 1

    pct_stats = results.get('short_interest_pct', {})
    days_stats = results.get('days_to_cover', {})
    _logger.info("Short Interest %%: %d records updated", pct_stats.get('updated_count', 0))
    _logger.info("Days to Cover: %d records updated", days_stats.get('updated_count', 0))
    _logger.info("Volume Data: %d tickers fetched", results.get('volume_data_count', 0))

    if 'duration' in results:
        _logger.info("Duration: %.2f seconds", results['duration'])

    _logger.info("Population completed successfully")
    return 0
