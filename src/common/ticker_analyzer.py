...
from src.common.fundamentals import format_fundamental_analysis, get_fundamentals_unified
from src.common.technicals import format_technical_analysis
from src.common.ticker_chart import generate_chart
from src.model.telegram_bot import Technicals, TickerAnalysis
...
async def analyze_ticker(
    ...
    try:
        ...
        if provider.lower() in ["fmp", "yf", "av", "fh", "td", "pg"]:
            try:
                fundamentals = await get_fundamentals_unified(
                    ticker, provider, force_refresh=force_refresh_fundamentals
                )
            except Exception:
                _logger.exception(
                    "Error getting fundamentals using unified function, falling back to traditional function"
                )
                fundamentals = None
        ...
        try:
            ...
            if len(close) < 50:
                _logger.warning(
                    "Insufficient data for indicator calculations: %d points (need at least 50)", len(close)
                )
            ...
            try:
                ...
                if current_indicators.get("bb_upper") is not None and current_indicators.get("bb_lower") is not None:
                    ...
                    rec = recommendation_engine.get_recommendation(
                        "BB_MIDDLE", current_indicators["bb_middle"], context
                    )
...
def format_ticker_report(analysis: TickerAnalysis) -> dict:
    ...
    if analysis.fundamentals:
        ...
        company_name = (
            getattr(analysis.fundamentals, "company_name", None)
            or getattr(analysis.fundamentals, "longName", None)
            or getattr(analysis.fundamentals, "name", None)
            or getattr(analysis.fundamentals, "shortName", None)
        )
    ...
    return {"message": full_msg.strip(), "chart_bytes": chart_bytes}
