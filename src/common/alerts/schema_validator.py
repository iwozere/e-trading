...
class AlertSchemaValidator:
    ...
    def _validate_config(self, task_params: Dict[str, Any], schema_type: str) -> ValidationResult:
        ...
        try:
            ...
            _logger.error("Unexpected error during validation: %s", str(e))
            return ValidationResult(
                is_valid=False,
    ...
    def load_schema(self, job_type: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            _logger.error("Invalid JSON in schema file %s: %s", schema_file, str(e))
            return None
        except jsonschema.SchemaError as e:
            _logger.error("Invalid schema in file %s: %s", schema_file, str(e))
            return None
        except Exception as e:
            _logger.error("Error loading schema from %s: %s", schema_file, str(e))
            return None
