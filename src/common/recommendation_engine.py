...
class FundamentalRecommendationRules:
    ...
    def get_dividend_yield_recommendation(value: float) -> Tuple[RecommendationType, float, str]:
        ...
        if value >= 4.0:
            return RecommendationType.BUY, 0.7, "High dividend yield - Good income"
        elif value >= 2.0:
            return RecommendationType.HOLD, 0.5, "Moderate dividend yield - Decent income"
        else:
            return RecommendationType.SELL, 0.6, "Low dividend yield - Poor income"
    ...
    def get_growth_recommendation(value: float) -> Tuple[RecommendationType, float, str]:
        ...
        if value >= 0.10:
            return RecommendationType.STRONG_BUY, 0.8, "High growth - Excellent prospects"
        elif value >= 0.05:
            return RecommendationType.BUY, 0.6, "Good growth - Strong prospects"
        elif value >= 0.0:
            return RecommendationType.HOLD, 0.5, "Stable growth - Moderate prospects"
        else:
            return RecommendationType.SELL, 0.7, "Negative growth - Poor prospects"
...
class RecommendationEngine:
    ...
    def __init__(self):
        ...
        self.fundamental_functions = {
            ...
            "QUICK_RATIO": self.fundamental_rules.get_current_ratio_recommendation,
    ...
    def get_recommendation(self, indicator: str, value: float, context: Dict = None) -> Recommendation:
        ...
        try:
            ...
            _logger.error(f"Error getting recommendation for {indicator}: {e}")
            return Recommendation(
    ...
    def get_composite_recommendation(self, indicator_set: IndicatorSet) -> CompositeRecommendation:
        ...
        try:
            ...
            _logger.error(f"Error calculating composite recommendation: {e}")
            return CompositeRecommendation(
                recommendation=RecommendationType.HOLD,
                confidence=0.0,
