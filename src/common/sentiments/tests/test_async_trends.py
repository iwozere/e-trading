...
class TestAsyncTrendsAdapter:
    ...
    async def adapter(self):
        ...
        adapter = AsyncTrendsAdapter(concurrency=1, rate_limit_delay=0.01, max_retries=1, session=mock_session)
        yield adapter
        await adapter.close()
    ...
    async def test_fetch_summary_success(
        self, adapter, sample_interest_over_time_response, sample_related_queries_response
    ):
