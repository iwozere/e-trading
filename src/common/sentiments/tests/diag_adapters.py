...
async def diagnose():
    ...
    ticker = "AAPL"
    since_ts = int((datetime.now(UTC) - timedelta(hours=24)).timestamp())
    ...
    for provider in providers:
        ...
        try:
            ...
            if summary:
                print(
                    f"SUCCESS: {provider} found {summary.get('mentions', 0)} mentions. Score: {summary.get('sentiment_score', 0.0)}"
                )
            else:
