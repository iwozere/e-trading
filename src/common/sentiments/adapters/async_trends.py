...
class AsyncTrendsAdapter(BaseSentimentAdapter):
    ...
    async def _get_data(self, url: str, params: dict, referer: str) -> dict | None:
        session = await self._get_session()
    ...
    async def _fetch_tokens(self, ticker: str):
        if self._tokens and time.time() < self._token_expiry:
            return True
        ...
        data = await self._get_data(
            "https://trends.google.com/trends/api/explore",
            {
                ...
                "req": json.dumps(
                    {
                        "comparisonItem": [{"keyword": ticker.upper(), "geo": "US", "time": "now 7-d"}],
                        "category": 0,
                        "property": "",
                    },
                    separators=(",", ":"),
        ...
        if data and "widgets" in data:
            self._tokens = {
                w["id"]: {"token": w["token"], "request": w.get("request")} for w in data["widgets"] if "token" in w
            }
            self._token_expiry = time.time() + 1800
            return True
        return False

    async def fetch_messages(self, ticker: str, since_ts: int | None = None, limit: int = 100) -> List[Dict]:
        if not await self._fetch_tokens(ticker):
            return []
                ...
                messages.append(
                    {
                        ...
                        "created_at": datetime.fromtimestamp(int(pt.get("time", 0))).isoformat()
                        if pt.get("time")
                        else "",
    ...
    async def fetch_summary(self, ticker: str, since_ts: int | None = None) -> Dict:
        # Lazy load analyzer
