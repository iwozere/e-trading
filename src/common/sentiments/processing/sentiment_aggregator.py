...
class AggregatedSentiment:
    ...
    temporal_trend: str | None  # 'improving', 'declining', 'stable'
...
class SentimentAggregator:
    ...
    def __init__(self, config: Dict | None = None):
        ...
        # Quality weights
        self.quality_weights = {"excellent": 1.0, "good": 0.8, "fair": 0.6, "poor": 0.3}
    ...
    def aggregate_sentiment(self, sources: List[SourceSentiment], method: str | None = None) -> AggregatedSentiment:
        """
    ...
    def _remove_outliers(self, sources: List[SourceSentiment]) -> List[SourceSentiment]:
        ...
        for source in sources:
            ...
            if z_score <= self.outlier_threshold:
                ...
                _logger.debug("Removing outlier source %s with z-score %.2f", source.source_name, z_score)
    ...
    def _weighted_average_aggregation(self, sources: List[SourceSentiment], weights: Dict[str, float]) -> float:
    ...
    def _calculate_confidence(self, sources: List[SourceSentiment], weights: Dict[str, float]) -> float:
        ...
        # Combine factors
        final_confidence = source_count_factor * 0.3 + avg_confidence * 0.4 + sample_factor * 0.2 + avg_quality * 0.1
    ...
    def _calculate_confidence_interval(self, sources: List[SourceSentiment], final_score: float) -> Tuple[float, float]:
    ...
    def _analyze_temporal_trend(self, sources: List[SourceSentiment]) -> str | None:
        ...
        cutoff_time = datetime.now(UTC) - timedelta(hours=self.trend_window_hours)
