...
class HeuristicSentimentAnalyzer:
    ...
    def _load_sentiment_keywords(self) -> None:
        ...
        _logger.debug(
            "Loaded %d positive and %d negative keywords", len(self.positive_keywords), len(self.negative_keywords)
        )
    ...
    def _load_emoji_mappings(self) -> None:
        ...
        default_emoji_sentiment = {"🚀": 1.0, "💎": 0.9, "🌙": 0.8, "📈": 0.9, "📉": -0.9, "💀": -1.0}
    ...
    def _load_slang_mappings(self) -> None:
        ...
        default_slang_sentiment = {"hodl": 0.8, "lambo": 0.8, "rekt": -0.9, "fud": -0.6}
    ...
    def analyze_trend_queries(self, queries: List[str]) -> Dict[str, int]:
        ...
        for query in queries:
            ...
            bullish_found = any(
                all(term in query_lower for term in term_group) for term_group in trend_terms.get("bullish", [])
            )
            bearish_found = any(
                all(term in query_lower for term in term_group) for term_group in trend_terms.get("bearish", [])
            )
