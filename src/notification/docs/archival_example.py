...
def example_basic_archival_usage(session: Session):
    ...
    _logger.info(
        "- Failed messages ready for cleanup: %d", stats["current_status"]["failed_messages_ready_for_cleanup"]
    )
    ...
    if archival_service.is_low_traffic_period():
        ...
        for operation, result in results.items():
            _logger.info(
                "Operation '%s': %d messages processed",
                operation,
...
def example_retention_policy_usage(session: Session):
    ...
    for name, policy in policies.items():
        _logger.info(
            "- %s: archive after %d days, delete after %d days",
            name,
...
def example_scheduled_cleanup_usage(session: Session):
    ...
    for task in status["tasks"]:
        _logger.info(
            "  - Task '%s': interval %d hours, next run %s", task["name"], task["interval_hours"], task["next_run"]
        )
...
async def example_scheduled_archival_daemon(session: Session):
    ...
    try:
        ...
        await asyncio.wait_for(archival_service.run_scheduled_archival(interval_hours=1), timeout=30.0)
    except TimeoutError:
...
def example_manual_archival_utility(session: Session):
    ...
    for operation, stats in results.items():
        _logger.info(
            "- %s: %d messages, %d bytes, %.2f seconds",
            operation,
