...
class MockChannel(NotificationChannel):
    ...
    async def send_message(
        ...
        if self.should_fail:
            ...
            return DeliveryResult(
                success=False, status=DeliveryStatus.BOUNCED, error_message=f"Mock failure for {self.channel_name}"
            )
        else:
            return DeliveryResult(
                success=True,
    ...
    async def check_health(self) -> ChannelHealth:
        return ChannelHealth(
            status=ChannelHealthStatus.HEALTHY, last_check=datetime.now(UTC), response_time_ms=50
        )
...
def sample_message_content():
    ...
    return MessageContent(text="Test message", subject="Test Subject", metadata={"test": "data"})


@pytest.fixture
def channel_instances():
    """Create mock channel instances."""
    return {"telegram": MockChannel("telegram", {}), "email": MockChannel("email", {}), "sms": MockChannel("sms", {})}
...
class TestFallbackRule:
    ...
    def test_invalid_fallback_rule_empty_fallbacks(self):
        ...
        with pytest.raises(ValueError, match="At least one fallback channel must be specified"):
            FallbackRule(primary_channel="telegram", fallback_channels=[], strategy=FallbackStrategy.PRIORITY_ORDER)
...
class TestFallbackManager:
    ...
    def test_configure_fallback_rule(self, fallback_manager):
        ...
        rule = FallbackRule(
            primary_channel="telegram", fallback_channels=["email", "sms"], strategy=FallbackStrategy.PRIORITY_ORDER
        )
    ...
    def test_remove_fallback_rule(self, fallback_manager):
        ...
        rule = FallbackRule(
            primary_channel="telegram", fallback_channels=["email"], strategy=FallbackStrategy.PRIORITY_ORDER
        )
    ...
    async def test_retry_queue_processing(self, fallback_manager, sample_message_content):
        ...
        failed_msg = FailedMessage(
            ...
            failed_at=datetime.now(UTC) - timedelta(minutes=2),  # Old enough to retry
            retry_count=0,
        )
        ...
        channel_instances = {"telegram": MockChannel("telegram", {}), "email": MockChannel("email", {})}
    ...
    def test_dead_letter_queue_management(self, fallback_manager, sample_message_content):
        ...
        for i in range(5):
            failed_msg = FailedMessage(
                ...
                failed_at=datetime.now(UTC) - timedelta(hours=i),
            )
    ...
    async def test_manual_reprocessing(self, fallback_manager, sample_message_content):
        ...
        success, message = await fallback_manager.reprocess_dead_letter_message(
            message_id=1, channel_instances=channel_instances
        )
    ...
    async def test_cleanup_old_dead_letters(self, fallback_manager, sample_message_content):
        ...
        old_msg = FailedMessage(
            ...
            failed_at=datetime.now(UTC) - timedelta(days=35),  # Older than retention
        )
        ...
        new_msg = FailedMessage(
            ...
            failed_at=datetime.now(UTC) - timedelta(days=5),  # Within retention
        )
