...
@pytest.mark.parametrize(
    "func,command,extra_args,result_key,result_value,expected_type,expected_title,expected_message",
    [
...
async def test_simple_commands_success(
    mocker, func, command, extra_args, result_key, result_value, expected_type, expected_title, expected_message
):
    message = mocker.Mock()
    message.chat.id = 1
    ...
    with patch("src.telegram.screener.notifications.handle_command") as mock_handle_command:
        mock_handle_command.return_value = {"status": "ok", result_key: result_value, "title": expected_title}
...
async def test_simple_commands_error(
    mocker, func, command, extra_args, result_key, result_value, expected_type, expected_title, expected_message
):
    message = mocker.Mock()
    message.chat.id = 1
    ...
    notification_manager.send_notification = AsyncMock()
    with patch("src.telegram.screener.notifications.handle_command") as mock_handle_command:
        mock_handle_command.return_value = {"status": "error", result_key: result_value, "title": expected_title}
        if extra_args is not None:
            await func(message, telegram_user_id, extra_args, notification_manager)
        else:
            await func(message, telegram_user_id, notification_manager)
...
@pytest.mark.asyncio
@pytest.mark.parametrize(
    "func,command,args,result_key,result_value,expected_type,expected_title,expected_message",
    [
        (
            process_register_command,
            "register",
            ["register", "user@example.com", "en"],
            "message",
            "Registered!",
            "INFO",
            "Register",
...
async def test_commands_with_args(
    mocker, func, command, args, result_key, result_value, expected_type, expected_title, expected_message
):
    message = mocker.Mock()
    message.chat.id = 1
    message.message_id = 2
    ...
    with patch("src.telegram.screener.notifications.handle_command") as mock_handle_command:
        ...
        mock_handle_command.return_value = {"status": status, result_key: result_value, "title": expected_title}
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            {
                "status": "ok",
                "reports": [{"ticker": "mrns", "error": "Ticker not found", "message": "", "chart_bytes": None}],
                "email": False,
                "user_email": None,
            },
        ...
        "reports": [
            {
                "ticker": "vt",
                "error": None,
                "message": f"VT report {provider} {period} {interval}",
                "chart_bytes": b"bytes",
            },
            {
                "ticker": "vti",
                "error": None,
                "message": f"VTI report {provider} {period} {interval}",
                "chart_bytes": b"bytes",
            },
        ],
        "email": True,
