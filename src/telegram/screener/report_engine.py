...
class ReportEngine:
    ...
    async def handle_report(self, parsed: ParsedCommand) -> Dict[str, Any]:
        ...
        if config_json:
            try:
                ...
                if not is_valid:
                    return {"status": "error", "title": "Report Error", "message": f"Invalid config: {'; '.join(errors)}"}
            ...
            if not tickers: return {"status": "error", "message": "No tickers specified"}
        ...
        if telegram_user_id and self.telegram_service:
            ...
            if status: user_email = status.get("email")
        ...
        for ticker in tickers:
            ...
            if not analysis.error: all_failed = False

        if all_failed:
            return {"status": "error", "title": "Report Error", "message": f"No data retrieved for {', '.join(tickers)}."}
    ...
    async def analyze_ticker_business(self, ticker: str, provider: str = None, period: str = "2y", interval: str = "1d", force_refresh: bool = True) -> TickerAnalysis:
        """Fetch OHLCV and return TickerAnalysis."""
        try:
            return await analyze_ticker(ticker=ticker, provider=provider, period=period, interval=interval, force_refresh=force_refresh)
        except Exception as e:
            _logger.exception(f"Error analyzing {ticker}")
            return TickerAnalysis(ticker=ticker.upper(), provider=provider, period=period, interval=interval, error=str(e))
