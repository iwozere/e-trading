...
class ScreenerEngine:
    ...
    async def handle_screener(self, parsed: ParsedCommand) -> Dict[str, Any]:
        ...
        try:
            ...
            if config_json.startswith('{'):
                ...
                if not is_valid: return {"status": "error", "message": f"Invalid config: {errors}"}
                ...
                if not screener_config:
                    return {"status": "error", "message": f"Unknown screener: {config_json}. Available: {', '.join(self._get_available_screeners())}"}
            ...
            if report.error: return {"status": "error", "message": report.error}
    ...
    def _get_list_type_for_screener(self, screener_name: str) -> str:
        if screener_name == "six_stocks": return "swiss_shares"
        elif screener_name == "mid_cap_stocks": return "us_medium_cap"
        elif screener_name in ["large_cap_stocks", "extra_large_cap_stocks"]: return "us_large_cap"
    ...
    def _get_technical_criteria_for_screener(self, screener_name: str):
        return [{"indicator": "RSI", "parameters": {"period": 14}, "condition": {"operator": "<", "value": 75}, "weight": 0.5, "required": False}]
