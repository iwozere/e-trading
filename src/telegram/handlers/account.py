...
async def process_register_command_immediate(user_id: str, parsed: ParsedCommand, message: Message) -> None:
    ...
    try:
        ...
        try:
            ...
            if codes_sent >= 5:
                await message.reply(
                    "❌ **Rate Limit Exceeded**\n\nToo many verification codes sent. Please wait an hour before requesting another.",
                    parse_mode="Markdown",
                )
                return
        ...
        if existing_user:
            ...
            if current_email and current_email.lower() == email.lower():
                if existing_user.get("verified"):
                    await message.reply(
                        f"ℹ️ **Email Already Registered & Verified**\n\n📧 Your email **{email}** is already registered and verified\n✅ Status: Verified\n\n💡 No changes needed. Use `/info` to see your account details.",
                        parse_mode="Markdown",
                    )
                    return
                else:
                ...
                try:
                    ...
                    await message.reply(
                        f"✅ **Email Updated!**\n\n📧 Your email has been updated to **{email}**\n📨 A new verification code has been sent\n\nUse `/verify CODE` to verify your new email address.",
                        parse_mode="Markdown",
                    )
                except Exception as exc:
...
async def process_verify_command_immediate(user_id: str, parsed: ParsedCommand, message: Message) -> None:
    ...
    try:
        ...
        if not user_status:
            await message.reply(
                "❌ User not found. Please register first using `/register your@email.com`", parse_mode="Markdown"
            )
            return
        ...
        if success:
            await message.reply(
                "✅ **Email Verified Successfully!**\n\nYour email has been verified. You can now use all bot features including email reports.",
                parse_mode="Markdown",
            )
        else:
            await message.reply(
                "❌ **Invalid or Expired Code**\n\nPlease check the code or request a new one with `/register`",
                parse_mode="Markdown",
            )
    except Exception:
...
async def process_language_command_immediate(user_id: str, parsed: ParsedCommand, message: Message) -> None:
    ...
    try:
        ...
        if not user_status.get("approved", False):
            await message.reply(
                "❌ **Access Restricted**\n\nThis feature requires admin approval. Please use `/request_approval` first.",
                parse_mode="Markdown",
            )
            return
        ...
        if success:
            await message.reply(
                f"✅ **Language Updated**\n\nYour language preference has been updated to **{language.upper()}**.",
                parse_mode="Markdown",
            )
        else:
...
async def process_request_approval_command_immediate(user_id: str, parsed: ParsedCommand, message: Message) -> None:
    ...
    try:
        ...
        await message.reply(
            "✅ **Approval Request Submitted**\n\nYour request has been submitted to the administrators. You will be notified when your request is reviewed.",
            parse_mode="Markdown",
        )
        ...
        try:
            ...
            if not client:
                _logger.warning(
                    "Notification client unavailable — admin approval request not forwarded for user %s", user_id
                )
            else:
                try:
