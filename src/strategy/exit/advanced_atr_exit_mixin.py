...
class AdvancedATRExitMixin(BaseExitMixin):
    ...
    def __init__(self, params: Dict[str, Any] | None = None):
        ...
        self.anchor: str = self._resolve_param("anchor", "x_anchor", "high")
        self.update_on: str = self._resolve_param("update_on", "x_update_on", "bar_close")
        self.k_init: float = self._resolve_param("k_init", "x_k_init", 2.5)
        self.k_run: float = self._resolve_param("k_run", "x_k_run", 2.0)
        self.k_phase2: float = self._resolve_param("k_phase2", "x_k_phase2", 1.5)

        # ATR parameters
        self.p_fast: int = self._resolve_param("p_fast", "x_p_fast", 7)
        self.p_slow: int = self._resolve_param("p_slow", "x_p_slow", 21)
        self.use_htf_atr: bool = self._resolve_param("use_htf_atr", "x_use_htf_atr", True)
        self.htf: str = self._resolve_param("htf", "x_htf", "4h")
        self.alpha_fast: float = self._resolve_param("alpha_fast", "x_alpha_fast", 1.0)
        self.alpha_slow: float = self._resolve_param("alpha_slow", "x_alpha_slow", 1.0)
        self.alpha_htf: float = self._resolve_param("alpha_htf", "x_alpha_htf", 1.0)
        self.atr_floor: float = self._resolve_param("atr_floor", "x_atr_floor", 0.0)

        # Break-even and phases
        self.arm_at_R: float = self._resolve_param("arm_at_R", "x_arm_at_R", 1.0)
        self.breakeven_offset_atr: float = self._resolve_param("breakeven_offset_atr", "x_breakeven_offset_atr", 0.0)
        self.phase2_at_R: float = self._resolve_param("phase2_at_R", "x_phase2_at_R", 2.0)

        # Structural ratchet
        self.use_swing_ratchet: bool = self._resolve_param("use_swing_ratchet", "x_use_swing_ratchet", True)
        self.swing_lookback: int = self._resolve_param("swing_lookback", "x_swing_lookback", 10)
        self.struct_buffer_atr: float = self._resolve_param("struct_buffer_atr", "x_struct_buffer_atr", 0.25)

        # Time-based tightening
        self.tighten_if_stagnant_bars: int = self._resolve_param(
            "tighten_if_stagnant_bars", "x_tighten_if_stagnant_bars", 20
        )
        self.tighten_k_factor: float = self._resolve_param("tighten_k_factor", "x_tighten_k_factor", 0.8)
        self.min_bars_between_tighten: int = self._resolve_param("min_bars_between_tighten", "x_min_bars_between_tighten", 5)

        # Noise and step filters
        self.min_stop_step: float = self._resolve_param("min_stop_step", "x_min_stop_step", 0.0)
        self.noise_filter_atr: float = self._resolve_param("noise_filter_atr", "x_noise_filter_atr", 0.0)
        self.max_trail_freq: int = self._resolve_param("max_trail_freq", "x_max_trail_freq", 1)
    ...
    def should_exit(self) -> bool:
        ...
        if self.strategy is None or not self.strategy.position:
            return False
