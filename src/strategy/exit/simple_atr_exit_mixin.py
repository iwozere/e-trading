...
class SimpleATRExitMixin(BaseExitMixin):
    ...
    def on_entry(self, entry_price: float, direction: str, size: float, entry_reason: str):
        ...
        logger.debug(f"SimpleATRExitMixin.on_entry: price={entry_price}, direction={direction}, size={size}")
        ...
        if self.atr_name in self.indicators:
            ...
            if atr_value is not None and atr_value > 0:
                ...
                logger.debug(f"Initial ATR stop set: {self.current_stop} (ATR: {atr_value}, multiplier: {atr_multiplier})")
            else:
    ...
    def should_exit(self) -> bool:
        ...
        try:
                ...
                if entry_price is not None:
                    ...
                    if self.strategy.position.size > 0:  # long position
                        if current_price >= entry_price + profit_threshold:
                            ...
                            logger.debug(f"Moved to breakeven: {self.current_stop}")
                    else:  # short position
                        if current_price <= entry_price - profit_threshold:
                            self.current_stop = entry_price
                            self.breakeven_triggered = True
            ...
            if self.current_stop is not None:
                ...
                if self.strategy.position.size > 0:  # long position
                    ...
                    if new_stop > self.current_stop:
                        ...
                        logger.debug(f"Updated long stop: {self.current_stop}")
                else:  # short position
                    new_stop = current_price + (atr_value * atr_multiplier)
                    if new_stop < self.current_stop:
                        self.current_stop = new_stop
                        logger.debug(f"Updated short stop: {self.current_stop}")
            ...
            if self.current_stop is not None:
                ...
                if should_exit:
                    logger.debug(f"ATR exit triggered: price={current_price}, stop={self.current_stop}")
