...
def objective(trial: optuna.trial.Trial, df: pd.DataFrame):
    ...
    trial_df, _ = calculate_technicals_from_df(trial_df, indicators=indicators_to_add, indicator_params=indicator_params)
...
def run_experiment(csv_file: Path):
    ...
    final_df, _ = calculate_technicals_from_df(final_df, indicators=indicators_to_add, indicator_params=best_indicator_params)
