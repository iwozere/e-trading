...
class IndicatorAdapter(Protocol):
    ...
    async def compute(
        self, name: str, df: pd.DataFrame, inputs: Dict[str, pd.Series], params: Dict[str, Any]
    ) -> Dict[str, pd.Series]:
...
class RecommendationProvider(Protocol):
    ...
    def get_recommendation(
        self, indicator_name: str, value: Union[float, Dict[str, float]], context: Dict[str, Any] | None = None
    ) -> Any:
...
class ParameterValidator(BaseModel):
    ...
    timeperiod: int | None = Field(None, ge=1, le=200)
    fastperiod: int | None = Field(None, ge=1, le=100)
    slowperiod: int | None = Field(None, ge=1, le=200)
    signalperiod: int | None = Field(None, ge=1, le=50)
    nbdevup: float | None = Field(None, ge=0.1, le=5.0)
    nbdevdn: float | None = Field(None, ge=0.1, le=5.0)
...
def is_technical_indicator_result(result: Any) -> bool:
    ...
    return (
        ...
        and result.category == "technical"
        and hasattr(result, "value")
        and is_valid_indicator_value(result.value)
    )
...
def is_fundamental_indicator_result(result: Any) -> bool:
    ...
    return (
        hasattr(result, "category")
        and result.category == "fundamental"
        and hasattr(result, "value")
        and is_valid_indicator_value(result.value)
    )
...
class BaseIndicatorCalculator(ABC):
    ...
    @abstractmethod
    async def calculate(self, data: pd.DataFrame, parameters: Dict[str, Any]) -> Union[float, Dict[str, float]]:
...
class BaseRecommendationEngine(ABC):
    ...
    @abstractmethod
    def get_composite_recommendation(self, indicators: Dict[str, Any]) -> Any:
...
__all__ = [
    ...
    "is_valid_ohlcv_data",
    "is_valid_indicator_value",
    "validate_indicator_parameters",
    "is_technical_indicator_result",
    "is_fundamental_indicator_result",
    # Abstract base classes
    "BaseIndicatorCalculator",
    "BaseRecommendationEngine",
    # Callback types
    "IndicatorCalculationCallback",
    "ErrorCallback",
    "ProgressCallback",
]
