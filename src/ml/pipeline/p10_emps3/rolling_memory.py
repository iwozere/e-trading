...
class EMPS3RollingMemoryScanner:
    def __init__(
        self, config: EMPS3RollingMemoryConfig, results_base_path: Path, target_date: str, verbose: bool = True
    ):
        self.config = config
    ...
    def detect_phase1_5_candidates(self, historical_df: pd.DataFrame) -> pd.DataFrame:
        ...
        freq_df = valid_df.groupby("ticker").agg({"scan_date": ["count", "min", "max"]}).reset_index()
    ...
    def generate_outputs(self, phase1_5_df: pd.DataFrame, output_dir: Path) -> Dict[str, Path]:
