...
class FeatureEngineer:
    ...
    def handle_missing_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        _logger.info(f"Handling missing data using strategy: {strategy}")
        ...
        if strategy == 'forward_fill':
            ...
            _logger.warning(f"Unknown missing data strategy: {strategy}, using forward_fill")
            df = df.fillna(method='ffill')

        # Remove any remaining NaN values at the beginning
        df = df.dropna()

        _logger.info(f"Missing data handling completed. DataFrame shape: {df.shape}")
        return df
    ...
    def handle_outliers(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        _logger.info(f"Handling outliers using strategy: {strategy}")
    ...
    def normalize_data(self, df: pd.DataFrame, scaler=None) -> Tuple[pd.DataFrame, object]:
        ...
        _logger.info(f"Normalizing data using method: {normalization_method}")
        ...
        if scaler is None:
            if normalization_method == 'minmax':
                ...
                _logger.warning(f"Unknown normalization method: {normalization_method}, using MinMaxScaler")
                scaler = MinMaxScaler()
    ...
    def prepare_sequences(self, df: pd.DataFrame, target_column: str = 'close') -> Tuple[np.ndarray, np.ndarray]:
        ...
        _logger.info(f"Preparing sequences with time_steps: {time_steps}")
        ...
        _logger.info(f"Sequence preparation completed. X shape: {X.shape}, y shape: {y.shape}")
        return X, y
    ...
    def split_data(self, X: np.ndarray, y: np.ndarray) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
        ...
        _logger.info(f"Splitting data with test_split: {test_split}, validation_split: {validation_split}")
        ...
        _logger.info(f"Data splitting completed:")
        _logger.info(f"  Train: {X_train.shape[0]} samples")
        _logger.info(f"  Validation: {X_val.shape[0]} samples")
        _logger.info(f"  Test: {X_test.shape[0]} samples")
    ...
    def process_single_file(self, filepath: Path) -> Dict[str, any]:
        ...
        """
        try:
            _logger.info(f"Processing file: {filepath.name}")
            ...
            _logger.info(f"Successfully processed {filepath.name}")
            ...
            _logger.error(f"Error processing {filepath.name}: {str(e)}")
            return {
                'success': False,
                'filename': filepath.name,
                'error': str(e)
    ...
    def run(self) -> Dict[str, List[Dict]]:
        ...
        _logger.info(f"Found {len(csv_files)} CSV files to process")
        ...
        _logger.info(f"  Successfully processed: {len(successful_processing)} files")
        _logger.info(f"  Failed processing: {len(failed_processing)} files")
...
def main():
    ...
    try:
        ...
        _logger.error("Feature engineering failed: %s", str(e))
        sys.exit(1)
