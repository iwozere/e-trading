...
class PipelineRunner:
    ...
    def _validate_config(self) -> bool:
        ...
        if missing_sections:
            _logger.error(f"Missing configuration sections: {missing_sections}")
            return False
        ...
        if missing_paths:
            _logger.error(f"Missing path configurations: {missing_paths}")
            return False
    ...
    def _run_stage_1_data_loading(self) -> Dict[str, Any]:
        ...
        try:
            ...
            _logger.info(f"Data loading completed: {len(results['success'])} successful, {len(results['failed'])} failed")
            ...
            _logger.error(f"Data loading failed: {str(e)}")
            return {
                'success': False,
                'error': str(e),
    ...
    def _run_stage_2_feature_engineering(self) -> Dict[str, Any]:
        ...
        try:
            ...
            _logger.info(f"Feature engineering completed: {len(results['success'])} successful, {len(results['failed'])} failed")
            ...
            _logger.error(f"Feature engineering failed: {str(e)}")
            return {
                'success': False,
                'error': str(e),
                'traceback': traceback.format_exc()
    ...
    def _run_stage_3_cnn_lstm_optimization(self) -> Dict[str, Any]:
        ...
        try:
            ...
            _logger.info(f"CNN-LSTM optimization completed: Best trial {study.best_trial.number}")
            ...
            _logger.error(f"CNN-LSTM optimization failed: {str(e)}")
            return {
                'success': False,
                'error': str(e),
                'traceback': traceback.format_exc()
    ...
    def _run_stage_4_cnn_lstm_training(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if results['success']:
                _logger.info(f"CNN-LSTM training completed successfully")
                _logger.info(f"Model saved to: {results['model_path']}")
                _logger.info(f"Test metrics: {results['metrics']}")
            else:
                _logger.error(f"CNN-LSTM training failed: {results['error']}")

            return results

        except Exception as e:
            _logger.error(f"CNN-LSTM training failed: {str(e)}")
            return {
                'success': False,
                'error': str(e),
                'traceback': traceback.format_exc()
    ...
    def _run_stage_5_feature_extraction(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if results['success']:
                _logger.info(f"Feature extraction completed successfully")
                _logger.info(f"Summary: {results['summary']}")
            else:
                _logger.error(f"Feature extraction failed: {results['error']}")

            return results

        except Exception as e:
            _logger.error(f"Feature extraction failed: {str(e)}")
            return {
                'success': False,
                'error': str(e),
                'traceback': traceback.format_exc()
    ...
    def _run_stage_6_xgboost_optimization(self) -> Dict[str, Any]:
        ...
        try:
            ...
            _logger.info(f"XGBoost optimization completed successfully")
            _logger.info(f"Best validation MSE: {results['best_validation_mse']:.6f}")
            ...
            _logger.error(f"XGBoost optimization failed: {str(e)}")
            return {
                'success': False,
                'error': str(e),
                'traceback': traceback.format_exc()
    ...
    def _run_stage_7_xgboost_training(self) -> Dict[str, Any]:
        ...
        try:
            ...
            _logger.info(f"XGBoost training completed successfully")
            _logger.info(f"Test MSE: {results['test']['mse']:.6f}")
            _logger.info(f"Test Directional Accuracy: {results['test']['directional_accuracy']:.4f}")
            ...
            _logger.error(f"XGBoost training failed: {str(e)}")
            return {
                'success': False,
                'error': str(e),
                'traceback': traceback.format_exc()
    ...
    def _run_stage_8_model_validation(self) -> Dict[str, Any]:
        ...
        try:
            ...
            _logger.info(f"Model validation completed successfully")
            _logger.info(f"Validation report generated")
            ...
            _logger.error(f"Model validation failed: {str(e)}")
            return {
                'success': False,
                'error': str(e),
                'traceback': traceback.format_exc()
    ...
    def run_stage(self, stage_number: int) -> Dict[str, Any]:
        ...
        _logger.info(f"Running stage {stage_number}: {stage_name}")
        ...
        try:
            ...
            if result['success']:
                ...
                _logger.info(f"Stage {stage_number} completed successfully in {result['execution_time']:.2f} seconds")
            else:
                self.pipeline_state['failed_stages'].append(stage_number)
                _logger.error(f"Stage {stage_number} failed after {result['execution_time']:.2f} seconds")
            ...
            _logger.error(f"Stage {stage_number} failed with exception after {execution_time:.2f} seconds: {str(e)}")
    ...
    def run_stages(self, stage_numbers: List[int]) -> Dict[str, Any]:
        ...
        _logger.info(f"Running stages: {stage_numbers}")

        results = {}

        for stage_number in stage_numbers:
            if stage_number in self.pipeline_state['completed_stages']:
                _logger.info(f"Stage {stage_number} already completed, skipping")
                continue
            ...
            if not result['success'] and not self.config.get('pipeline', {}).get('continue_on_failure', False):
                _logger.error(f"Stage {stage_number} failed, stopping pipeline")
                break
    ...
    def run(self, start_stage: int = 1, end_stage: int = 8, skip_stages: List[int] = None) -> Dict[str, Any]:
        ...
        _logger.info(f"Pipeline stages to run: {stages_to_run}")
        ...
        _logger.info("=" * 80)
        _logger.info(f"Overall Success: {summary['success']}")
        _logger.info(f"Total Execution Time: {total_time:.2f} seconds")
        _logger.info(f"Completed Stages: {summary['completed_stages']}")
        _logger.info(f"Failed Stages: {summary['failed_stages']}")
...
def main():
    ...
    try:
        ...
        _logger.error(f"Pipeline execution failed: {str(e)}")
