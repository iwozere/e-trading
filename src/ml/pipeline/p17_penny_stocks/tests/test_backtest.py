...
def test_keeps_earliest_detection(tmp_path):
    _write_day(tmp_path, "2026-06-01", [{"ticker": "AAA", "price": 2.0, "tier": "B",
                                         "company_name": "A", "final_score": 60}])
    _write_day(tmp_path, "2026-06-02", [{"ticker": "AAA", "price": 9.0, "tier": "A",
                                         "company_name": "A", "final_score": 80}])
...
def test_since_and_tier_filters_and_bad_price(tmp_path):
    ...
    _write_day(tmp_path, "2026-06-05", [
...
def test_backtest_ticker_math():
    ...
    rec = {"ticker": "AAA", "detection_date": "2026-06-01", "detection_price": 2.0,
           "tier": "B", "company_name": "A", "final_score": 60}
    out = bt.backtest_ticker(_FakeDownloader(df), rec, invest=1000.0,
                             end_date=pd.Timestamp("2026-06-10").to_pydatetime())
...
def test_backtest_ticker_no_data():
    rec = {"ticker": "AAA", "detection_date": "2026-06-01", "detection_price": 2.0,
           "tier": "B", "company_name": "A", "final_score": 60}
    out = bt.backtest_ticker(_FakeDownloader(pd.DataFrame()), rec, 1000.0,
                             pd.Timestamp("2026-06-10").to_pydatetime())
