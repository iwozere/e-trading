...
class DailyDeepScan:
    ...
    def run_deep_scan(self, candidates: Optional[List[Candidate]] = None) -> DeepScanResults:
        ...
        try:
            ...
            _logger.error("Error in daily deep scan run: %s", e)
            raise
    ...
    def _load_active_candidates(self) -> List[Candidate]:
        ...
        try:
            ...
            _logger.error("Error loading active candidates: %s", e)
            return []
    ...
    def _convert_db_candidate_to_model(self, db_candidate) -> Optional[Candidate]:
        ...
        try:
            ...
            # Extract structural metrics from database model
            structural_metrics = StructuralMetrics(
                short_interest_pct=float(db_candidate.short_interest_pct),
                days_to_cover=float(db_candidate.days_to_cover),
                float_shares=int(db_candidate.float_shares),
                avg_volume_14d=int(db_candidate.avg_volume_14d),
                market_cap=int(db_candidate.market_cap)
            )

            candidate = Candidate(
                ticker=db_candidate.ticker,
                screener_score=float(db_candidate.screener_score),
                structural_metrics=structural_metrics,
                last_updated=db_candidate.last_updated,
            ...
            _logger.warning("Error converting DB candidate %s: %s",
                          getattr(db_candidate, 'ticker', 'Unknown'), e)
            return None
    ...
    def _convert_db_adhoc_to_candidate(self, db_adhoc) -> Optional[Candidate]:
        ...
        try:
            ...
            candidate = Candidate(
                ...
                structural_metrics=structural_metrics,
                last_updated=db_adhoc.first_seen,
            ...
            _logger.warning("Error converting DB ad-hoc candidate %s: %s",
                          getattr(db_adhoc, 'ticker', 'Unknown'), e)
            return None
    ...
    def _process_batch(self, candidates: List[Candidate],
        ...
        try:
            ...
            _logger.error("Error processing batch: %s", e)
            return []
    ...
    def _scan_candidate(self, candidate: Candidate,
        ...
        try:
            ...
            # Calculate final squeeze score (placeholder - will be implemented in scoring engine)
            squeeze_score = self._calculate_preliminary_squeeze_score(
                candidate.screener_score, transient_metrics
            )
            ...
            _logger.debug("Scanned %s: squeeze_score=%.3f, volume_spike=%.2f, sentiment=%.2f",
    ...
    def _calculate_preliminary_squeeze_score(self, screener_score: float,
        ...
        """
        Calculate preliminary squeeze score combining screener and transient metrics.
        ...
        try:
            ...
            # Combine with screener score (70% transient, 30% structural)
            final_score = 0.7 * transient_score + 0.3 * screener_score
    ...
    def _store_results(self, scored_candidates: List[ScoredCandidate],
        ...
        try:
            ...
            _logger.error("Error storing deep scan results: %s", e)
            raise
