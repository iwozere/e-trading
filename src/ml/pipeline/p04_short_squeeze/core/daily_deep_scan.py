...
class DailyDeepScan:
    ...
    def __init__(
        ...
        self.use_enhanced_sentiment = SENTIMENT_MODULE_AVAILABLE and sentiment_config is not None
    ...
    def run_deep_scan(self, candidates: List[Candidate] | None = None) -> DeepScanResults:
        """
        ...
        try:
            ...
            for i in range(0, len(candidates), batch_size):
                ...
                _logger.info(
                    "Processed batch %d-%d: %d candidates scored",
                    i + 1,
            ...
            _logger.info(
                "Daily deep scan completed: %d candidates processed, %d scored", len(candidates), len(scored_candidates)
            )
    ...
    def _load_active_candidates(self) -> List[Candidate]:
        ...
        try:
            ...
            _logger.info(
                "Loaded %d active candidates (%d from screener, %d ad-hoc)",
                len(all_candidates),
                len(screener_candidates),
                len(adhoc_candidates),
            )
    ...
    def _convert_db_candidate_to_model(self, db_candidate) -> Candidate | None:
        ...
        try:
            ...
            _logger.warning("Error converting DB candidate %s: %s", ticker if "ticker" in locals() else "Unknown", e)
            return None

    def _convert_db_adhoc_to_candidate(self, db_adhoc) -> Candidate | None:
        ...
        try:
            ...
            _logger.warning(
                "Error converting DB ad-hoc candidate %s: %s", ticker if "ticker" in locals() else "Unknown", e
            )
            return None

    def _enhance_candidate_with_finra_data(self, candidate: Candidate, metrics: Dict[str, Any]) -> Candidate:
        """
        ...
        try:
            ...
            _logger.debug(
                "Enhanced %s with FINRA data: SI=%.1f%%, DTC=%.1f, age=%d days",
                ticker,
            ...
            _logger.warning("Error enhancing candidate %s with FINRA data: %s", candidate.ticker, e)
            return candidate

    async def _get_historical_mentions_async(self, ticker: str) -> float | None:
        """
    ...
    def _collect_batch_sentiment(self, candidates: List[Candidate], metrics: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            _logger.info("Batch sentiment collection complete: %d/%d successful", success_count, len(ticker_list))
            ...
            for ticker, features in sentiment_map.items():
                if features:
                    ...
                    if features.mentions_growth_7d and features.mentions_growth_7d > 2.0:
                        _logger.info(
                            "  %s: MENTION SURGE detected (+%.0f%%)", ticker, features.mentions_growth_7d * 100
                        )
                    if features.bot_pct > 0.5:
                        _logger.warning("  %s: High bot activity detected (%.0f%%)", ticker, features.bot_pct * 100)
    ...
    def _process_batch(self, candidates: List[Candidate], metrics: Dict[str, Any]) -> List[ScoredCandidate]:
        """
        ...
        try:
            ...
            for candidate in candidates:
                try:
                    ...
                    scored_candidate = self._scan_candidate(candidate, metrics, sentiment_map.get(candidate.ticker))
                    if scored_candidate:
    ...
    def _scan_candidate(
        self, candidate: Candidate, metrics: Dict[str, Any], sentiment_features: Any | None = None
        ...
        try:
            ...
            _logger.debug(
                "Scanned %s: squeeze_score=%.3f, volume=%.2f, sentiment=%.2f, virality=%.2f, SI=%.1f%%, DTC=%.1f",
                ticker,
                squeeze_score,
                transient_metrics.volume_spike,
                transient_metrics.sentiment_24h,
                transient_metrics.virality_index,
    ...
    def calculate_transient_metrics(
        self, ticker: str, metrics: Dict[str, Any], sentiment_features: Any | None = None
    ...
    def _calculate_volume_spike_ratio(self, ticker: str) -> float | None:
        """
        ...
        try:
            ...
            _logger.debug(
                "Volume spike for %s: current=%d, avg_14d=%d, ratio=%.2f",
                ticker,
                current_volume,
                avg_volume_14d,
                volume_spike,
    ...
    def _get_sentiment_score(self, ticker: str) -> float | None:
        """
    ...
    def _get_call_put_ratio(self, ticker: str) -> float | None:
        """
    ...
    def _get_borrow_fee_percentage(self, ticker: str) -> float | None:
        """
    ...
    def _calculate_preliminary_squeeze_score(
        ...
        try:
            ...
            final_score = 0.5 * transient_score + 0.3 * finra_score + 0.2 * screener_score
    ...
    def _store_results(
        ...
        try:
            ...
            _logger.info("Stored deep scan results in database: %d candidates", len(scored_candidates))
...
if __name__ == "__main__":
    ...
    if fmp_downloader.test_connection():
        ...
        top_candidates = sorted(results.scored_candidates, key=lambda c: c.squeeze_score, reverse=True)[:5]

        for i, scored_candidate in enumerate(top_candidates, 1):
            candidate = scored_candidate.candidate
            metrics = scored_candidate.transient_metrics
            print(
                f"  {i}. {candidate.ticker}: squeeze_score={scored_candidate.squeeze_score:.3f}, "
