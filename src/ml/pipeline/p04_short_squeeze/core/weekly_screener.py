...
class WeeklyScreener:
    ...
    def __init__(self, fmp_downloader: FMPDataDownloader, config: ScreenerConfig):
        ...
        _logger.info("Weekly Screener initialized with run_id=%s", self.run_id)
    ...
    def run_screener(self, universe: List[str]) -> ScreenerResults:
        ...
        try:
            ...
            for i, ticker in enumerate(universe, 1):
                try:
                    candidate = self._screen_ticker(ticker, data_quality_metrics)
                    if candidate:
                    ...
                    data_quality_metrics['failed_fetches'] += 1
                    continue
            ...
            _logger.error("Error in weekly screener run: %s", e)
            raise

    def _screen_ticker(self, ticker: str, metrics: Dict[str, Any]) -> Optional[Candidate]:
        """
    ...
    def _filter_candidates(self, candidates: List[Candidate]) -> List[Candidate]:
        ...
        try:
            ...
            _logger.error("Error filtering candidates: %s", e)
            return candidates
    ...
    def _select_top_candidates(self, candidates: List[Candidate]) -> List[Candidate]:
        ...
        try:
            ...
            _logger.error("Error selecting top candidates: %s", e)
    ...
    def _store_results(self, candidates: List[Candidate], data_quality_metrics: Dict[str, Any],
        ...
        try:
            ...
            _logger.error("Error storing screener results: %s", e)
            raise
