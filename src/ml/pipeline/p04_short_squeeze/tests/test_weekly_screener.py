...
from src.ml.pipeline.p04_short_squeeze.core.weekly_screener import (
    ScreenerResults,
    WeeklyScreener,
...
class TestWeeklyScreener(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.screener_config = ScreenerConfig(
            filters=ScreenerFilters(
                si_percent_min=0.15, days_to_cover_min=5.0, float_max=100_000_000, top_k_candidates=10
            ),
            scoring=ScreenerWeights(short_interest_pct=0.4, days_to_cover=0.3, float_ratio=0.2, volume_consistency=0.1),
        )
    ...
    def test_screen_ticker_success(self):
        ...
        # Mock API responses
        self.mock_fmp_downloader.get_short_interest_data.return_value = {"shortInterest": 10_000_000}
        ...
        mock_df = pd.DataFrame({"volume": [500_000] * 20})
        self.mock_fmp_downloader.get_ohlcv.return_value = mock_df
