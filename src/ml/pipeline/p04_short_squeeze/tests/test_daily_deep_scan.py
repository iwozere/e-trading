...
from src.ml.pipeline.p04_short_squeeze.core.daily_deep_scan import (
    DailyDeepScan,
    DeepScanResults,
    create_daily_deep_scan,
)
...
class TestDailyDeepScan(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.deep_scan_config = DeepScanConfig(
            ...
            metrics=DeepScanMetrics(volume_lookback_days=14, sentiment_lookback_hours=24, options_min_volume=100),
            scoring=DeepScanWeights(volume_spike=0.35, sentiment_24h=0.25, call_put_ratio=0.20, borrow_fee=0.20),
        )

        self.daily_deep_scan = DailyDeepScan(
            self.mock_fmp_downloader, self.mock_finnhub_downloader, self.deep_scan_config
        )
    ...
    def test_calculate_volume_spike_ratio_success(self):
        ...
        mock_df = pd.DataFrame(
            {"volume": volumes, "timestamp": pd.date_range(start="2024-01-01", periods=15, freq="D")}
        )
    ...
    def test_get_call_put_ratio_success(self):
        ...
        # Mock options data
        mock_options = {"total_call_volume": 1000, "total_put_volume": 500, "call_put_volume_ratio": 2.0}
    ...
    def test_calculate_transient_metrics_success(self):
        ...
        # Mock sentiment data
        self.mock_finnhub_downloader.aggregate_24h_sentiment.return_value = {"sentiment_score_24h": 0.4}
        ...
        # Mock borrow rates data
        self.mock_finnhub_downloader.get_borrow_rates_data.return_value = {"fee_rate_percentage": 8.0}
    ...
    def test_scan_candidate_success(self):
        ...
        with patch.object(self.daily_deep_scan, "calculate_transient_metrics") as mock_calc:
            mock_transient = TransientMetrics(
                volume_spike=2.5, call_put_ratio=1.8, sentiment_24h=0.3, borrow_fee_pct=4.5
            )
        ...
        def mock_scan_candidate(candidate, metrics):
            if candidate.ticker in ["TEST1", "TEST2"]:
                ...
                return ScoredCandidate(
                    candidate=candidate, transient_metrics=transient_metrics, squeeze_score=0.75, alert_level=None
                )
            return None
    ...
    def test_store_results(self, mock_session_scope):
        ...
        with patch("src.ml.pipeline.p04_short_squeeze.core.daily_deep_scan.ShortSqueezeService") as mock_service_class:
            ...
            scored_candidate = ScoredCandidate(
                candidate=candidate, transient_metrics=transient_metrics, squeeze_score=0.8, alert_level=None
            )
    ...
    def test_run_deep_scan_with_candidates(self, mock_session_scope):
        ...
        with patch("src.ml.pipeline.p04_short_squeeze.core.daily_deep_scan.ShortSqueezeService") as mock_service_class:
            ...
            # Create test candidates
            candidates = [self.create_test_candidate("TEST1", 0.7), self.create_test_candidate("TEST2", 0.8)]
