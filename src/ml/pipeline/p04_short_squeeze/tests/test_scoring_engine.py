...
class TestScoringEngine(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.scoring_config = ScoringConfig(
            normalization_method="minmax", score_bounds=(0.0, 1.0), weight_validation=True
        )

        self.deep_scan_weights = DeepScanWeights(
            volume_spike=0.35, sentiment_24h=0.25, call_put_ratio=0.20, borrow_fee=0.20
        )
        ...
        self.transient_metrics = TransientMetrics(
            volume_spike=3.5, call_put_ratio=2.1, sentiment_24h=0.6, borrow_fee_pct=0.15
        )
    ...
    def test_calculate_squeeze_score(self):
        ...
        score = self.scoring_engine.calculate_squeeze_score(self.structural_metrics, self.transient_metrics)
    ...
    def test_extract_transient_metrics_with_none_values(self):
        ...
        metrics_with_none = TransientMetrics(
            volume_spike=2.0, call_put_ratio=None, sentiment_24h=0.3, borrow_fee_pct=None
        )
    ...
    def test_minmax_normalize_volume_spike(self):
        ...
        # Test normal range
        self.assertAlmostEqual(self.scoring_engine._minmax_normalize("volume_spike", 1.0), 0.0)
        self.assertAlmostEqual(self.scoring_engine._minmax_normalize("volume_spike", 10.0), 1.0)
        self.assertAlmostEqual(self.scoring_engine._minmax_normalize("volume_spike", 5.5), 0.5)

        # Test clamping
        self.assertEqual(self.scoring_engine._minmax_normalize("volume_spike", 0.5), 0.0)
        self.assertEqual(self.scoring_engine._minmax_normalize("volume_spike", 15.0), 1.0)

    def test_minmax_normalize_sentiment(self):
        """Test min-max normalization for sentiment."""
        # Sentiment is already normalized [-1, 1] -> [0, 1]
        self.assertAlmostEqual(self.scoring_engine._minmax_normalize("sentiment_24h", -1.0), 0.0)
        self.assertAlmostEqual(self.scoring_engine._minmax_normalize("sentiment_24h", 1.0), 1.0)
        self.assertAlmostEqual(self.scoring_engine._minmax_normalize("sentiment_24h", 0.0), 0.5)

    def test_sigmoid_normalize(self):
        """Test sigmoid normalization."""
        # Test volume spike sigmoid
        sigmoid_config = ScoringConfig(normalization_method="sigmoid", score_bounds=(0.0, 1.0), weight_validation=True)
    ...
    def test_normalize_metrics(self):
        ...
        metrics = {"volume_spike": 3.5, "sentiment_24h": 0.6, "call_put_ratio": 2.1, "borrow_fee": 0.15}
    ...
    def test_apply_weights(self):
        ...
        normalized_metrics = {"volume_spike": 0.6, "sentiment_24h": 0.7, "call_put_ratio": 0.5, "borrow_fee": 0.4}
    ...
    def test_score_candidate(self):
        ...
        scored_candidate = self.scoring_engine.score_candidate(self.candidate, self.transient_metrics)
    ...
    def test_get_score_breakdown(self):
        ...
        breakdown = self.scoring_engine.get_score_breakdown(self.structural_metrics, self.transient_metrics)
        ...
        required_keys = [
            ...
            "structural_score",
            "transient_score",
            "raw_transient_metrics",
            "normalized_transient_metrics",
            "weights",
            "combination_weights",
        ]
    ...
    def test_scoring_consistency(self):
        ...
        score1 = self.scoring_engine.calculate_squeeze_score(self.structural_metrics, self.transient_metrics)

        score2 = self.scoring_engine.calculate_squeeze_score(self.structural_metrics, self.transient_metrics)
    ...
    def test_scoring_with_extreme_values(self):
        ...
        high_score = self.scoring_engine.calculate_squeeze_score(self.structural_metrics, extreme_high_transient)
        ...
        low_score = self.scoring_engine.calculate_squeeze_score(self.structural_metrics, extreme_low_transient)
    ...
    def test_invalid_normalization_method(self):
        ...
        invalid_config = ScoringConfig(
            normalization_method="invalid_method", score_bounds=(0.0, 1.0), weight_validation=True
        )
        ...
        score = invalid_engine.calculate_squeeze_score(self.structural_metrics, self.transient_metrics)
