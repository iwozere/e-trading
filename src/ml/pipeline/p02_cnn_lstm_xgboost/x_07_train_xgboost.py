...
class XGBoostTrainer:
    ...
    def save_checkpoint(self, model: xgb.XGBRegressor, iteration: int, train_metrics: dict, val_metrics: dict):
    ...
    def _create_plots(self, y_true: np.ndarray, y_pred: np.ndarray, title: str, save_path: Path):
    ...
    def train(self) -> dict:
        ...
        # Create model
        model = xgb.XGBRegressor(**best_params, random_state=42, n_jobs=-1, verbosity=0, early_stopping_rounds=50)
        ...
        final_iteration = getattr(model, "n_estimators", None) or 100
        ...
        self._create_plots(
            y_val, y_val_pred, "XGBoost Validation Set Performance", self.visualizations_dir / "xgboost_val_plots.png"
        )

        self._create_plots(
            y_test, y_test_pred, "XGBoost Test Set Performance", self.visualizations_dir / "xgboost_test_plots.png"
        )
