...
class CNNLSTMOptimizer:
    ...
    def train_model(self, model: nn.Module, X_train: np.ndarray, y_train: np.ndarray,
        ...
        for epoch in range(epochs):
            ...
            if (epoch + 1) % 10 == 0:
                _logger.info(f'Epoch [{epoch+1}/{epochs}], Train Loss: {train_losses[-1]:.6f}, Val Loss: {val_losses[-1]:.6f}')
    ...
    def objective(self, trial: optuna.Trial) -> float:
        ...
        try:
            ...
            _logger.error(f"Error in trial: {str(e)}")
            raise optuna.TrialPruned()
    ...
    def run_optimization(self) -> optuna.Study:
        ...
        _logger.info(f"Best trial: {study.best_trial.number}")
        _logger.info(f"Best value: {study.best_trial.value}")
        _logger.info(f"Best params: {study.best_trial.params}")
    ...
    def _save_study_results(self, study: optuna.Study):
        ...
        _logger.info(f"Study results saved to {self.studies_dir}")
    ...
    def _create_optimization_plots(self, study: optuna.Study, timestamp: str):
        ...
        try:
            ...
            _logger.info(f"Optimization plots saved to {self.studies_dir}")

        except Exception as e:
            _logger.warning(f"Could not create optimization plots: {str(e)}")
...
def main():
    ...
    try:
        ...
        _logger.error("CNN-LSTM optimization failed: %s", str(e))
        sys.exit(1)
