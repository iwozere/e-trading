...
class PipelineRunner:
    ...
    def _run_stage_1_data_loading(self) -> Dict[str, Any]:
        ...
        try:
            ...
            _logger.info(
                "Data loading completed: %d successful, %d failed", len(results["success"]), len(results["failed"])
            )
    ...
    def _run_stage_2_feature_engineering(self) -> Dict[str, Any]:
        ...
        try:
            ...
            _logger.info(
                "Feature engineering completed: %d successful, %d failed",
                len(results["success"]),
                len(results["failed"]),
            )
    ...
    def _run_stage_8_model_validation(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"success": True, "results": results, "message": "Model validation completed successfully"}
    ...
    def run_stage(self, stage_number: int) -> Dict[str, Any]:
        ...
        if stage_number not in self.stages:
            return {"success": False, "error": f"Invalid stage number: {stage_number}"}
...
def main():
    ...
    try:
        ...
        results = runner.run(start_stage=start_stage, end_stage=end_stage, skip_stages=skip_stages)
