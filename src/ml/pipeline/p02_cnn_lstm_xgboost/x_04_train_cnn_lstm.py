...
class CNNLSTMTrainer:
    ...
    def train_model(self, model: nn.Module) -> Tuple[nn.Module, Dict[str, List[float]]]:
        ...
        for epoch in range(epochs):
            ...
            # Logging
            if (epoch + 1) % 10 == 0:
                _logger.info(f'Epoch [{epoch+1}/{epochs}], Train Loss: {avg_train_loss:.6f}, Val Loss: {val_loss:.6f}, LR: {optimizer.param_groups[0]["lr"]:.6f}')

        # Load best model
        if best_model_state is not None:
            model.load_state_dict(best_model_state)
            _logger.info(f"Loaded best model with validation loss: {best_val_loss:.6f}")
    ...
    def save_model(self, model: nn.Module, history: Dict[str, List[float]]):
        ...
        _logger.info(f"Training history saved to {history_path}")
        _logger.info(f"Model info saved to {info_path}")
    ...
    def create_training_plots(self, history: Dict[str, List[float]], save_path: Path):
        ...
        _logger.info(f"Training plots saved to {save_path}")
    ...
    def evaluate_model(self, model: nn.Module) -> Dict[str, float]:
        ...
        _logger.info(f"Test MSE: {test_loss:.6f}")
        _logger.info(f"Test MAE: {mae:.6f}")
        _logger.info(f"Test RMSE: {rmse:.6f}")
