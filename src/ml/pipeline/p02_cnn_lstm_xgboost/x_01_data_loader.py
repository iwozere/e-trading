        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for future in as_completed(future_to_task):
                ...
                try:
                    ...
                    failed_downloads.append(filename)
                    _logger.error("Exception downloading %s %s %s: %s", provider, symbol, timeframe, str(e))
    ...
    def validate_downloaded_data(self) -> Dict[str, List[str]]:
        ...
        for filepath in self.data_dir.glob("*.csv"):
            try:
                ...
                _logger.error("Error validating file %s: %s", filepath.name, str(e))
                invalid_files.append(filepath.name)
...
def main():
    ...
    try:
        ...
        _logger.error("Data loading failed: %s", str(e))
        sys.exit(1)
