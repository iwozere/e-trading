...
class HMMTrainer:
    ...
    def compute_indicators(
        self, df: pd.DataFrame, rsi_period: int, atr_period: int, bb_period: int, vol_period: int
    ) -> pd.DataFrame:
        """
    ...
    def evaluate_hmm_info_criteria(self, X: np.ndarray, n_states: int) -> Tuple[float, float, GaussianHMM]:
        ...
        """
        model = GaussianHMM(
            n_components=n_states, covariance_type=self.covariance_type, n_iter=self.n_iter, random_state=42
        )
        model.fit(X)
        logL = model.score(X)
    ...
    def map_states_to_regimes(
        self, df: pd.DataFrame, state_col: str = "state", ret_col: str = "log_return", atr_col: str = "atr"
        ...
        mapping = {
            int(row["state"]): regime_labels[i] if i < len(regime_labels) else regime_labels[-1]
    ...
    def plot_results(self, df: pd.DataFrame, file_name: str):
        ...
        color_map = {
            regime: regime_colors[int(regime)] if int(regime) < len(regime_colors) else "gray"
    ...
    def run_grid_search(self, df: pd.DataFrame, file_base: str) -> Tuple[pd.DataFrame, GaussianHMM, Tuple, List]:
        ...
        for i, (rsi_p, atr_p, bb_p, vol_p, n_states) in enumerate(
            product(rsi_grid, atr_grid, bb_grid, vol_grid, n_states_grid)
        ):
            try:
                ...
                _logger.warning(
                    "Failed to evaluate parameters (rsi=%d, atr=%d, bb=%d, vol=%d, states=%d): %s",
                    rsi_p,
                    atr_p,
                    bb_p,
                    vol_p,
                    ...
                    best_result.update(
                        {
                            "bic": bic,
                            "aic": aic,
                            "params": (rsi_p, atr_p, bb_p, vol_p, n_states),
                            "model": model,
                            "df": temp_df.copy(),
                        }
                    )
    ...
    def process_file(self, file_path: str) -> Dict:
        ...
        try:
            ...
            with open(model_file, "wb") as f:
                pickle.dump({"model": best_model, "params": best_params, "mapping": mapping, "file_base": file_base}, f)
            ...
            best_params_dict = {
                ...
                "model_performance": {"aic": best_aic, "bic": best_bic},
