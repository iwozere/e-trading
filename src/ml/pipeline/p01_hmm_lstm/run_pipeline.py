            ...
            if not stage_result["success"]:
                if stage["critical"]:
                    ...
                    if fail_fast:
                        ...
                        _logger.warning(
                            "Critical stage %d failed; continuing automatically (non-interactive mode)", stage_num
                        )
                else:
                    ...
                    if not continue_on_optional_failures and is_interactive:
                        ...
                        _logger.warning(
                            "Optional stage %d failed; stopping (continue_on_optional_failures=False)", stage_num
                        )
                        break
                    else:
    ...
    def _log_pipeline_summary(self, results: Dict) -> None:
        ...
        if failed_critical:
            ...
            for stage in failed_critical:
                _logger.error("  [CRITICAL FAILED] Stage %d: %s - %s", stage["stage"], stage["name"], stage["error"])

        if failed_optional:
            _logger.warning("\nFailed OPTIONAL stages:")
            for stage in failed_optional:
                _logger.warning("  [OPTIONAL FAILED] Stage %d: %s - %s", stage["stage"], stage["name"], stage["error"])
...
Examples:
  ...
  python run_pipeline.py --continue-on-optional-failures  # Continue on optional failures
    ...
    parser.add_argument("--config", type=str, default="config/pipeline/p01.yaml", help="Path to configuration file")
    parser.add_argument("--skip-stages", type=str, help='Comma-separated list of stages to skip (e.g., "1,2,3")')
    parser.add_argument("--symbols", type=str, help="Comma-separated list of symbols (overrides config)")
    parser.add_argument("--timeframes", type=str, help="Comma-separated list of timeframes (overrides config)")
    parser.add_argument("--list-stages", action="store_true", help="List all pipeline stages and exit")
    parser.add_argument(
        "--validate-only", action="store_true", help="Only validate requirements without running pipeline"
    )
    parser.add_argument(
        "--no-fail-fast", action="store_true", help="Disable fail-fast mode (continue on critical failures)"
    )
    parser.add_argument(
        "--continue-on-optional-failures", action="store_true", help="Continue pipeline even if optional stages fail"
    )
    parser.add_argument(
        "--interactive",
        action="store_true",
