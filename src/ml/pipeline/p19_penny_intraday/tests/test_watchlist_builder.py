...
def _builder(tmp_path, rows, date="2026-06-26", **cfg_over):
    ...
    for k, v in cfg_over.items():
        setattr(cfg, k, v)
    return WatchlistBuilder(cfg, date, p17_results_dir=p17, output_dir=str(tmp_path / "p19"))
...
def test_selects_tier_abc_and_explosive_only(tmp_path):
    rows = [
        ...
        _row("DDD", tier="W", explosive=True),
    ]  # included via explosive
...
def test_baseline_context_mapped(tmp_path):
    rows = [
        _row(
            ...
            dilution=20.0,
            catalyst="catalyst_tier1_news_2026-06-24",
            catalyst_score=90,
        )
    ]
...
def test_ranking_prefers_explosive_then_tier(tmp_path):
    rows = [_row("C1", tier="C"), _row("B1", tier="B"), _row("EXP", tier="C", explosive=True)]
...
def test_baseline_enrichment_fills_non_p17(tmp_path):
    ...
    b = WatchlistBuilder(
        cfg, "2026-06-26", p17_results_dir=p17, output_dir=str(tmp_path / "p19"), baseline_fetcher=fake_fetcher
    )
