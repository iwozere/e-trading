"""
Accumulation Analyzer (Stage 3 — accumulation mode)

Replaces VolatilityFilter when EMPS2PipelineConfig.analyzer_type == 'accumulation'.
Implements the Coiled Spring strategy formula.

Moved from p10_emps3/accumulation_analyzer.py.  P10 now imports from here.
"""
...
class AccumulationAnalyzer:
    ...
    Satisfies the ScreenerAnalyzer protocol via apply_filters().
    """
    ...
    def __init__(
        ...
        """
        Args:
            data_manager: DataManager instance for OHLCV downloads.
            config: EMPS2FilterConfig with accumulation-mode fields populated.
            results_dir: Directory for checkpoint and diagnostic output.
            target_date: Target trading date (YYYY-MM-DD). Defaults to today.
            chunk_size: Tickers per OHLCV download chunk (memory bound).
            checkpoint_enabled: Persist diagnostics after each chunk for resume.
        """
        ...
        _logger.info(
            "AccumulationAnalyzer initialized: AR>%.1f, Vol Z-Score>%.1f, chunk_size=%d, checkpoint=%s",
    ...
    def apply_filters(self, tickers: List[str]) -> pd.DataFrame:
        """
        Apply accumulation filter to a list of tickers.

        Returns a DataFrame with only PASSED tickers and their diagnostic metrics.
        """
        _logger.info("Applying AccumulationAnalyzer to %d tickers", len(tickers))
        ...
        if processed_tickers:
            _logger.info(
                ...
                len(tickers) - len(processed_tickers),
            )
        ...
        try:
                ...
                for ticker in chunk:
                    try:
                        ...
                        if len(df_intra) < 20 or len(df_daily) < 20:
                            diagnostic_data.append(
                                {"ticker": ticker, "status": "FAILED", "reason": "insufficient_bars"}
                            )
                            continue
                        ...
                        diagnostic_data.append({"ticker": ticker, "status": "ERROR", "reason": "unexpected_error"})
                    finally:
                        processed_tickers.add(ticker)
            ...
            _logger.warning("Interrupted; flushing checkpoint so the next run can resume.")
            self._save_checkpoint(diagnostic_data)
            raise
        except Exception:
    ...
    def _check_accumulation(self, ticker: str, df_intra: pd.DataFrame, df_daily: pd.DataFrame) -> tuple:
        ...
        price_range_1d = (
            (df_daily["high"].iloc[-1] - df_daily["low"].iloc[-1]) / df_daily["low"].iloc[-1]
            if df_daily["low"].iloc[-1] > 0
            else 0.0
        )
    ...
    def _coerce_ohlcv_timestamp_column(self, df: pd.DataFrame | None) -> pd.DataFrame | None:
    ...
    def _save_results(self, results_data: list) -> None:
        if not results_data:
            return
        df = pd.DataFrame(results_data)
        out = self._results_dir / "07_prebreakout_watchlist.csv"
        df.to_csv(out, index=False)
        high_priority_count = len(df[df.get("prebreakout_score", 0) > 70]) if "prebreakout_score" in df.columns else 0
