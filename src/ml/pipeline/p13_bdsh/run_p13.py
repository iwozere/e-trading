...
def main():
    ...
    parser.add_argument("--output", type=str, choices=["text", "json"], default="text", help="Output format for summary")
        ...
        if args.live:
            ...
            if args.output == "json":
                ...
                for item in summary_info:
                    print(f"Ticker: [{item['ticker']}] | Current VIX Z: [{item['vix_z']:.2f}] | Status: [{item['status']}] | Stop-Loss: [${item['stop_loss']:.2f}]")
                print("="*80 + "\n")
