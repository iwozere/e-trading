...
class P13Plotter:
    ...
    def plot_results(
        ...
        fig, (ax1, ax2, ax3) = plt.subplots(
            3, 1, figsize=(30, 20), sharex=True, gridspec_kw={"height_ratios": [2, 1, 1]}
        )
        ...
        ax3.plot(
            results.index, equity_growth, label="Strategy Equity ($1000 Invested)", color="forestgreen", linewidth=2.5
        )
        ax3.plot(
            results.index,
            buy_and_hold,
        ...
        ax3.fill_between(
            results.index, 1000, equity_growth, where=(equity_growth >= 1000), facecolor="green", alpha=0.1
        )
        ax3.fill_between(results.index, 1000, equity_growth, where=(equity_growth < 1000), facecolor="red", alpha=0.1)
