...
def plot_p09_results(
    ...
    ax1.plot(
        signal_df.index, signal_df["price_a"], label=f"Price A ({pair_name.split('_')[0]})", alpha=0.7, color="blue"
    )
    ax1.plot(
        signal_df.index, signal_df["price_b"], label=f"Price B ({pair_name.split('_')[1]})", alpha=0.7, color="orange"
    )
    ...
    if not trades_df.empty:
        ...
        if not exits.empty:
            ax1.scatter(
                exits["exit_time"], exits["exit_price_a"], marker="x", color="black", s=50, label="Exit", zorder=5
            )
