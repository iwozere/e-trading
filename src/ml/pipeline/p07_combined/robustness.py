...
class P07RobustnessChecker:
    ...
    def run_all_checks(self, ohlcv: pd.DataFrame, params: Dict[str, Any], backtest_res: Dict[str, Any]):
        ...
        results["wfa_details"] = [
            {"window": r["window"], "sharpe": r["sharpe"], "return": r["return"]} for r in wfa.get("oos_results", [])
        ]
        ...
        _logger.info(
            "Robustness checks completed for %s %s. Results saved to %s", self.ticker, self.timeframe, self.res_dir
        )
        return results
