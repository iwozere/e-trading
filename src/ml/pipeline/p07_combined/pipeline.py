...
class P07Pipeline:
    ...
    def __init__(
        self, result_root: Path = Path("results/p07_combined"), db_url: str | None = None, enable_mtf: bool = False
    ):
    ...
    def train_macro_regimes(self, anchor_date: pd.Timestamp | None = None, force: bool = False) -> bool:
        """
    ...
    def run_optimization(
        ...
        ticker: str,
        timeframe: str,
        df_enriched: "pd.DataFrame | list[pd.DataFrame]",
        n_trials: int = 500,
        start_date: str = "",
        end_date: str = "",
    ):
        ...
        study = optuna.create_study(
            study_name=study_name, storage=self.db_url, load_if_exists=True, direction="maximize"
        )

        enable_mtf = self.enable_mtf
        if len(study.trials) < n_trials:
            _logger.info(
                "Starting optimization for %s_%s %s (%d trials)...",
                ticker,
                timeframe,
                f"[{start_date}_{end_date}]" if start_date else "",
                n_trials,
            )
            study.optimize(
            ...
            _logger.info(
                "Optimization already sufficient for %s_%s %s (%d trials).",
                ticker,
                timeframe,
                f"[{start_date}_{end_date}]" if start_date else "",
                len(study.trials),
    ...
    def save_artifacts(
        ...
        ticker: str,
        timeframe: str,
        ohlcv_clean: "pd.DataFrame | list[pd.DataFrame]",
        params: Dict[str, Any],
        start_date: str = "",
        end_date: str = "",
    ) -> bool:
        ...
        try:
            ...
            if not pd.isna(wfe) and wfe < 0.5:
                _logger.warning(
                    "Strategy REJECTED for %s %s: WFE=%.3f < 0.50. completed.flag will NOT be written.",
                    ticker,
                    timeframe,
                    wfe,
                )
        ...
        viz.plot_prediction_diagnostics(model._map_labels(res["y_test"]).values, model.predict_proba(res["X_test"]))
    ...
    def run_robustness(self, ticker: str, timeframe: str):
        ...
        if (
            "monte_carlo" in summary
            and "shuffled_returns" in summary["monte_carlo"]
            and "random_skips" in summary["monte_carlo"]
        ):
            viz.plot_monte_carlo_distribution(
                summary["monte_carlo"]["shuffled_returns"], summary["monte_carlo"]["random_skips"]
            )
        else:
            _logger.warning(
                "Skipping Monte Carlo plots due to missing data: %s",
                summary.get("monte_carlo", {}).get("error", "Unknown error"),
            )
    ...
    def run_batch(self, ticker_files: List[Path], mode: str = "optimize", train_years: List[str] | None = None):
        ...
        for (ticker, timeframe), files in groups.items():
            ...
            if len(files) > 1:
                ...
                val_file = files[-1]
                _logger.info(
                    "Cross-File Setup for %s %s: Opt on %d files, Val on %s",
                    ticker,
                    timeframe,
                    len(opt_files),
                    val_file["path"].name,
                ...
                opt_files = files
                val_file = None
                _logger.info("Single-File Setup for %s %s: Opt on %s", ticker, timeframe, opt_files[0]["path"].name)
            ...
            try:
                ...
                if self.is_completed(ticker, timeframe, agg_start, agg_end):
                    _logger.info(
                        "Optimization already completed for aggregated %s %s (%s_%s)",
                        ticker,
                        timeframe,
                        agg_start,
                        agg_end,
                ...
                if val_file:
                    _logger.info(
                        "Running Out-of-Sample Validation for %s %s on %s", ticker, timeframe, val_file["path"].name
                    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--enable-mtf", action="store_true", default=False, help="Enable Multi-Timeframe (anchor TF) features"
    )
