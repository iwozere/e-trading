...
def test_objective_accesses_pf_val_not_pf_test():
    ...
    mock_pf_val  = type("PF", (), {"sharpe_ratio": lambda self: 1.2,
                                    "trades": type("T", (), {"count": lambda self: type("C", (), {"sum": lambda self: 50})()})()})()
    mock_pf_test = type("PF", (), {"sharpe_ratio": lambda self: 9.9,
    ...
    assert score < 5.0, (
        f"Objective score {score} looks like it came from pf_test (sharpe=9.9) "
        "instead of pf_val (sharpe=1.2)"
    )
