...
class OrderFlowDataIngestor:
    ...
    """
    def __init__(self, config: OrderFlowConfig, data_manager: Optional[DataManager] = None):
        self.config = config
    ...
    def fetch_unified_data(self, symbol: str) -> pd.DataFrame:
        ...
        # Join Open Interest
        if not oi.empty:
            oi = oi[['sumOpenInterest', 'sumOpenInterestValue']].rename(columns={
        ...
        if not ls_ratio.empty:
            ls_ratio = ls_ratio[['longShortRatio', 'longAccount', 'shortAccount']].rename(columns={
                'longShortRatio': 'ls_ratio',
                'longAccount': 'ls_long_acc',
                'shortAccount': 'ls_short_acc'
            })
