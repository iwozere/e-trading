...
def test_significant_sell_returned() -> None:
    edgar = _make_edgar_mock(
        form4_rows=[{
            "ticker": "AAPL", "issuer_cik": "100", "insider_name": "Tim Cook",
            "transaction_code": "S", "shares": 10000, "price_per_share": 180.0,
            "total_value_usd": 1_800_000, "filed_date": "2024-02-14",
        }],
        dg_rows=[],
    )
...
def test_small_sale_filtered_out() -> None:
    edgar = _make_edgar_mock(
        form4_rows=[{
            "ticker": "XYZ", "issuer_cik": "200", "insider_name": "CEO",
            "transaction_code": "S", "shares": 100, "price_per_share": 10.0,
            "total_value_usd": 1_000, "filed_date": "2024-02-14",
        }],
        dg_rows=[],
    )
    monitor = Form4Monitor(edgar_downloader=edgar, min_sale_value_usd=500_000)
...
def test_buy_transaction_excluded() -> None:
    edgar = _make_edgar_mock(
        form4_rows=[{
            "ticker": "MSFT", "issuer_cik": "300", "insider_name": "CFO",
            "transaction_code": "P", "shares": 5000, "price_per_share": 400.0,
            "total_value_usd": 2_000_000, "filed_date": "2024-02-14",
        }],
        dg_rows=[],
    )
    monitor = Form4Monitor(edgar_downloader=edgar, min_sale_value_usd=500_000)
...
def test_13dg_amendments_returned() -> None:
    edgar = _make_edgar_mock(
        form4_rows=[],
        dg_rows=[{
            "cik": "400", "entity_name": "Acme Capital", "accession_number": "0001234",
            "filed_date": "2024-02-14", "form_type": "SC 13D/A",
        }],
    )
