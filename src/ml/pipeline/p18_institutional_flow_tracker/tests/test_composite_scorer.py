...
def test_large_billion_dollar_exit_reaches_threshold(scorer: CompositeScorer) -> None:
    ...
    consensus = pd.DataFrame([{"ticker": "AAPL", "institution_count": 3, "total_value_sold_usd": 2_500_000_000, "avg_exit_pct": -0.8}])
...
def test_large_exit_tiers_are_graded() -> None:
    ...
    s_small = scorer.score(small, pd.DataFrame(), pd.DataFrame(), pd.DataFrame(), date(2024, 5, 1)).iloc[0]["total_score"]
...
def test_breadth_bonus_rewards_more_institutions() -> None:
    ...
    s_narrow = scorer.score(narrow, pd.DataFrame(), pd.DataFrame(), pd.DataFrame(), date(2024, 5, 1)).iloc[0]["total_score"]
    s_broad = scorer.score(broad, pd.DataFrame(), pd.DataFrame(), pd.DataFrame(), date(2024, 5, 1)).iloc[0]["total_score"]
...
def test_consensus_plus_volume_scores_higher(scorer: CompositeScorer) -> None:
    consensus = pd.DataFrame([{"ticker": "TSLA", "institution_count": 4, "total_value_sold_usd": 1_200_000_000, "avg_exit_pct": -0.6}])
