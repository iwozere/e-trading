...
def generate_enhanced_chart(ticker: str, technicals_data: dict = None) -> bytes:
    ...
    try:
        ...
        ax1.plot(df.index, close, label='Close Price', color='black', linewidth=1)
        ax1.plot(df.index, bb_upper, label='BB Upper', color='red', alpha=0.7, linewidth=0.8)
        ax1.plot(df.index, bb_middle, label='BB Middle', color='blue', alpha=0.7, linewidth=0.8)
        ax1.plot(df.index, bb_lower, label='BB Lower', color='red', alpha=0.7, linewidth=0.8)
        ax1.plot(df.index, sma_50, label='SMA 50', color='orange', alpha=0.8, linewidth=1.2)
        ax1.plot(df.index, sma_200, label='SMA 200', color='purple', alpha=0.8, linewidth=1.2)
        ...
        ax3.plot(df.index, macd, label='MACD', color='blue', linewidth=1)
        ax3.plot(df.index, macd_signal, label='Signal', color='red', linewidth=1)
        ax3.bar(df.index, macd_hist, label='Histogram', color='gray', alpha=0.6, width=0.8)
        ...
        ax4.plot(df.index, stoch_k, label='%K', color='blue', linewidth=1)
        ax4.plot(df.index, stoch_d, label='%D', color='red', linewidth=1)
        ...
        ax5.plot(df.index, adx, label='ADX', color='black', linewidth=1)
        ax5.plot(df.index, plus_di, label='+DI', color='green', linewidth=1)
        ax5.plot(df.index, minus_di, label='-DI', color='red', linewidth=1)
        ...
        # Volume bars
        ax6.bar(df.index, volume, alpha=0.6, color='lightblue', label='Volume')
        ax6.set_ylabel('Volume', fontsize=10, color='blue')
        ax6.tick_params(axis='y', labelcolor='blue')
        
        # OBV line
        ax6_twin.plot(df.index, obv, color='orange', linewidth=1, label='OBV')
