...
class IBKRBroker(EnhancedBaseBroker, PaperTradingMixin):
    ...
    def __init__(self, host: str = '127.0.0.1', port: Optional[int] = None,
        ...
        _logger.info(f"Enhanced IBKR broker initialized - Mode: {self.trading_mode.value}, "
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            _logger.info(f"Connected to IBKR - Mode: {self.trading_mode.value}")
            ...
            _logger.exception(f"Failed to connect to IBKR: {e}")
    ...
    async def disconnect(self) -> bool:
        ...
        try:
            ...
            _logger.exception(f"Error disconnecting from IBKR: {e}")
            return False
    ...
    async def _load_account_info(self):
        ...
        try:
            ...
            _logger.info(f"Loaded IBKR account info - Portfolio items: {len(self.portfolio_items)}, "
                        f"Positions: {len(self.ibkr_positions)}")

        except Exception as e:
            _logger.exception(f"Failed to load IBKR account info: {e}")
    ...
    async def _start_market_data_updates(self):
        ...
        try:
            def market_data_worker():
                ...
                while self.market_data_running and self.ib.isConnected():
                    try:
                        ...
                        _logger.exception(f"Error in market data worker: {e}")
                        time.sleep(5)
            ...
            _logger.exception(f"Failed to start market data updates: {e}")
    ...
    def _subscribe_market_data(self, symbol: str, asset_class: str = 'STK') -> bool:
        ...
        try:
            ...
            if ticker:
                ...
                _logger.debug(f"Subscribed to market data for {symbol}")
                return True

            return False

        except Exception as e:
            _logger.exception(f"Failed to subscribe to market data for {symbol}: {e}")
            return False
    ...
    async def place_order(self, order: Order) -> str:
        ...
        try:
            ...
            if not is_valid:
                ...
                _logger.warning(f"Order validation failed: {validation_message}")
            ...
            if not ibkr_validation[0]:
                ...
                _logger.warning(f"IBKR validation failed: {ibkr_validation[1]}")
                return order.order_id
            ...
            _logger.exception(f"Error placing order: {e}")
            order.status = OrderStatus.REJECTED
    ...
    async def _validate_ibkr_order(self, order: Order) -> Tuple[bool, str]:
        ...
        try:
            ...
            _logger.exception(f"Error validating IBKR order: {e}")
    ...
    async def _place_live_order(self, order: Order) -> str:
        ...
        try:
            ...
            _logger.info(f"Live order placed on IBKR: {order.order_id} -> {ibkr_order.orderId}")
            ...
            _logger.exception(f"Error placing live order: {e}")
            order.status = OrderStatus.REJECTED
        ...
        try:
            if self.paper_trading_enabled:
                ...
                if order_id in self.ibkr_orders:
                    ...
                    _logger.info(f"Live order cancelled: {order_id}")
                    return True
                else:
                    _logger.warning(f"Order {order_id} not found for cancellation")
                    return False

        except Exception as e:
            _logger.exception(f"Error cancelling order {order_id}: {e}")
            return False
    ...
    async def get_order_status(self, order_id: str) -> Optional[Order]:
        ...
        try:
            ...
            _logger.exception(f"Error getting order status for {order_id}: {e}")
            return None
    ...
    async def get_positions(self) -> Dict[str, Position]:
        ...
        try:
            ...
            _logger.exception(f"Error getting positions: {e}")
            return {}
    ...
    async def get_portfolio(self) -> Portfolio:
        ...
        try:
            ...
            _logger.exception(f"Error getting portfolio: {e}")
            return Portfolio(
                total_value=0.0,
                cash=0.0,
                positions={},
    ...
    async def get_account_info(self) -> Dict[str, Any]:
        ...
        try:
            ...
            _logger.exception(f"Error getting account info: {e}")
            return {'error': str(e)}
    ...
    async def get_contract_details(self, symbol: str, asset_class: str = 'STK') -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            _logger.exception(f"Error getting contract details for {symbol}: {e}")
            return None
    ...
    async def get_market_data(self, symbol: str, asset_class: str = 'STK') -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            _logger.exception(f"Error getting market data for {symbol}: {e}")
            return None
    ...
    async def get_historical_data(self, symbol: str, duration: str = '1 D',
        ...
        try:
            ...
            _logger.exception(f"Error getting historical data for {symbol}: {e}")
            return None
