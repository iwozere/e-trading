...
class BrokerHealthMonitor:
    ...
    def add_broker(self, broker_id: str, broker: BaseBroker):
        ...
        _logger.info(f"Added broker {broker_id} to health monitoring")
    ...
    def remove_broker(self, broker_id: str):
        ...
        if broker_id in self.health_metrics:
            ...
            _logger.info(f"Removed broker {broker_id} from health monitoring")
        ...
        if broker_id in self.health_metrics:
            ...
            for callback in self.health_callbacks:
                try:
                    ...
                    _logger.exception(f"Error in health callback: {e}")
    ...
    def _monitor_loop(self):
        ...
        while self.monitoring_active:
            try:
                ...
                _logger.exception(f"Error in health monitoring loop: {e}")
                time.sleep(5)
    ...
    def get_connection(self, broker_config: Dict[str, Any]) -> BaseBroker:
        ...
        with self.pool_lock:
            ...
            if pool_key in self.connections and self.connections[pool_key]:
                ...
                _logger.debug(f"Reused broker connection from pool: {pool_key}")
                return broker
            ...
            try:
                ...
                self.connection_usage[pool_key] = self.connection_usage.get(pool_key, 0) + 1
                _logger.debug(f"Created new broker connection: {pool_key}")
                return broker
    ...
    def return_connection(self, broker: BaseBroker):
        ...
        with self.pool_lock:
            ...
            if len(self.connections[pool_key]) < self.max_connections_per_type:
                ...
                _logger.debug(f"Returned broker connection to pool: {pool_key}")
            else:
                # Pool is full, disconnect the broker
                asyncio.create_task(broker.disconnect())
                _logger.debug(f"Pool full, disconnected broker: {pool_key}")
            ...
            for pool_key, brokers in self.connections.items():
                for broker in brokers:
                    try:
                        ...
                        _logger.exception(f"Error disconnecting broker in cleanup: {e}")
                brokers.clear()
...
class BrokerManager:
    ...
    async def add_broker(self, broker_id: str, broker_config: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            _logger.info(f"Added broker {broker_id} ({broker_config.get('type', 'unknown')})")
            return True

        except Exception as e:
            _logger.exception(f"Failed to add broker {broker_id}: {e}")
            return False
    ...
    async def remove_broker(self, broker_id: str) -> bool:
        ...
        try:
            if broker_id in self.brokers:
                ...
                _logger.info(f"Removed broker {broker_id}")
                return True

            return False

        except Exception as e:
            _logger.exception(f"Failed to remove broker {broker_id}: {e}")
            return False
    ...
    async def start_broker(self, broker_id: str) -> bool:
        ...
        if broker_id not in self.brokers:
            _logger.error(f"Broker {broker_id} not found")
            return False
        ...
        try:
            ...
            if connected:
                ...
                _logger.info(f"Started broker {broker_id}")
                return True
            else:
    ...
    async def stop_broker(self, broker_id: str) -> bool:
        ...
        try:
            ...
            if disconnected:
                ...
                _logger.info(f"Stopped broker {broker_id}")
                return True
            else:
                self.health_monitor.update_broker_status(broker_id, BrokerStatus.ERROR,
    ...
    async def restart_broker(self, broker_id: str) -> bool:
        ...
        """
        _logger.info(f"Restarting broker {broker_id}")
    ...
    def _on_broker_health_change(self, broker_id: str, metrics: BrokerHealthMetrics):
        ...
        if metrics.status == BrokerStatus.ERROR and metrics.error_count <= 3:
            _logger.warning(f"Broker {broker_id} in error state, attempting restart")
