...
class IBKRContractManager:
    ...
    def create_option_contract(
        self, symbol: str, expiry: str, strike: float, right: str, exchange: str = "SMART", currency: str = "USD"
    ) -> Option:
        """
    ...
    def create_future_contract(self, symbol: str, expiry: str, exchange: str, currency: str = "USD") -> Future:
        """
    ...
    def get_contract(self, symbol: str, asset_class: str = "STK", **kwargs) -> Contract | None:
        """
...
class IBKRCommissionCalculator:
    ...
    def calculate_forex_cost(self, symbol: str, quantity: float, is_base_currency: bool = True) -> Dict[str, float]:
        """
        ...
        return {"cost": cost, "spread_pips": spread_pips, "spread_decimal": spread_decimal, "pip_value": pip_value}
    ...
    def calculate_option_margin(
        ...
        if option_type.startswith("long_"):
            ...
            margin_requirement = max(
                margin_base + contract_value - out_of_money, contract_value + (underlying_value * 0.10)
            )
            buying_power_used = margin_requirement
            ...
            margin_requirement = max(
                margin_base + contract_value - out_of_money, contract_value + (strike * contracts * 100 * 0.10)
            )
            buying_power_used = margin_requirement
...
class IBKROrderValidator:
    ...
    def validate_stock_order(self, symbol: str, quantity: int, price: float | None = None) -> Tuple[bool, str]:
        """
    ...
    def validate_option_order(
        self, symbol: str, expiry: str, strike: float, right: str, contracts: int
    ) -> Tuple[bool, str]:
        """
