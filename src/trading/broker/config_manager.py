...
class ConfigTemplate:
    ...
    def add_template(self, name: str, config: Dict[str, Any]):
        ...
        _logger.info(f"Added configuration template: {name}")

    def remove_template(self, name: str) -> bool:
        """Remove a template."""
        if name in self.templates:
            del self.templates[name]
            _logger.info(f"Removed configuration template: {name}")
            return True
        return False
...
class EnvironmentManager:
        ...
        for env in Environment:
            ...
            if config_file.exists():
                try:
                    ...
                    _logger.info(f"Loaded {env.value} environment configuration")
                except Exception as e:
                    _logger.exception(f"Failed to load {env.value} config: {e}")

    def set_environment(self, environment: Environment):
        """Set the current environment."""
        self.current_environment = environment
        _logger.info(f"Set environment to {environment.value}")
    ...
    def save_environment_config(self, environment: Environment, config: Dict[str, Any]):
        ...
        try:
            ...
            _logger.info(f"Saved {environment.value} environment configuration")

        except Exception as e:
            _logger.exception(f"Failed to save {environment.value} config: {e}")
            raise
...
class ConfigManager:
        ...
        try:
            for config_file in self.config_dir.glob("*.json"):
                ...
                try:
                    ...
                    _logger.debug(f"Loaded configuration: {config_name}")

                except Exception as e:
                    _logger.exception(f"Failed to load config {config_file}: {e}")

        except Exception as e:
            _logger.exception(f"Error loading configurations: {e}")
    ...
    def create_configuration(self, name: str, config: Dict[str, Any],
        ...
        try:
            ...
            _logger.info(f"Created configuration: {name}")
            return True

        except Exception as e:
            _logger.exception(f"Failed to create configuration {name}: {e}")
            return False
    ...
    def update_configuration(self, name: str, config: Dict[str, Any],
        ...
        try:
            ...
            _logger.info(f"Updated configuration: {name}")
            return True

        except Exception as e:
            _logger.exception(f"Failed to update configuration {name}: {e}")
            return False
    ...
    def delete_configuration(self, name: str) -> bool:
        ...
        try:
            ...
            _logger.info(f"Deleted configuration: {name}")
            return True

        except Exception as e:
            _logger.exception(f"Failed to delete configuration {name}: {e}")
            return False
    ...
    def create_from_template(self, name: str, template_name: str,
        ...
        try:
            ...
            _logger.exception(f"Failed to create configuration from template: {e}")
            return False
    ...
    def rollback_configuration(self, name: str, version: str) -> bool:
        ...
        try:
            ...
            if not target_version:
                _logger.error(f"Version {version} not found for configuration {name}")
                return False

            # Load configuration from version file
            version_file = self.config_dir / "versions" / f"{name}_{version}.json"
            if not version_file.exists():
                _logger.error(f"Version file not found: {version_file}")
                return False
            ...
            _logger.exception(f"Failed to rollback configuration {name} to {version}: {e}")
            return False
    ...
    def import_configuration(self, name: str, config_str: str,
        ...
        try:
            ...
            _logger.exception(f"Failed to import configuration: {e}")
            return False
