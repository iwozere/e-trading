...
class PaperTradingMixin:
    ...
    def _initialize_paper_portfolio(self) -> None:
        ...
        _logger.info(f"Initialized paper trading portfolio with ${config.initial_balance:,.2f}")
    ...
    async def simulate_realistic_execution(self, order: Order, market_price: float) -> Tuple[bool, float, float, str]:
        ...
        if config.realistic_fills and random.random() < config.partial_fill_probability:
            ...
            _logger.debug(f"Partial fill: {executed_quantity:.4f} of {order.quantity:.4f}")
    ...
    async def paper_place_order(self, order: Order, market_price: float) -> str:
        ...
        if hasattr(self, 'validate_order'):
            ...
            if not is_valid:
                ...
                _logger.warning(f"Order rejected: {validation_message}")
        ...
        _logger.info(f"Paper order placed: {order.order_id} - {order.symbol} {order.side.value} "
                    f"{order.quantity} @ {order.price or 'MARKET'} - Status: {order.status.value}")
    ...
    async def _process_paper_limit_order(self, order: Order, market_price: float) -> None:
        ...
        if ((order.side == OrderSide.BUY and market_price <= order.price) or
            ...
            _logger.debug(f"Limit order pending: {order.order_id} - Market: {market_price}, Limit: {order.price}")
    ...
    async def _process_paper_stop_order(self, order: Order, market_price: float) -> None:
        ...
        order.status = OrderStatus.PENDING
        _logger.debug(f"Stop order pending: {order.order_id} - Market: {market_price}, Stop: {order.stop_price}")
    ...
    async def _fill_paper_order(self, order: Order, executed_price: float,
        ...
        _logger.info(f"Paper order filled: {order.order_id} - {executed_quantity:.4f} @ ${executed_price:.4f} "
    ...
    async def paper_cancel_order(self, order_id: str) -> bool:
        ...
        _logger.info(f"Paper order cancelled: {order_id}")
        return True
