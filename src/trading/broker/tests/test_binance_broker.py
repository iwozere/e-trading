...
from src.trading.broker.base_broker import Order, OrderSide, OrderStatus, OrderType, TradingMode
...
class TestBinanceBroker:
    ...
    def live_config(self):
        ...
        return {
            ...
            "risk_management": {"max_position_size": 500.0, "max_daily_loss": 250.0},
            "notifications": {
                "position_opened": True,
                "position_closed": True,
                "email_enabled": False,  # Disable for tests
    ...
    async def test_paper_order_placement_market(self, paper_broker):
        ...
        order = Order(symbol="BTCUSDT", side=OrderSide.BUY, order_type=OrderType.MARKET, quantity=0.001)
    ...
    async def test_live_order_placement(self, live_broker):
        ...
        mock_response = {"orderId": 12345, "symbol": "BTCUSDT", "status": "NEW", "clientOrderId": order.client_order_id}
    ...
    async def test_paper_portfolio_tracking(self, paper_broker):
        ...
        buy_order = Order(symbol="BTCUSDT", side=OrderSide.BUY, order_type=OrderType.MARKET, quantity=0.001)
    ...
    async def test_websocket_message_processing(self, paper_broker):
        ...
        # Test ticker message
        ticker_message = {"s": "BTCUSDT", "c": "50000.00", "P": "2.5", "v": "1000.0"}
    ...
    async def test_pending_order_processing(self, paper_broker):
        ...
        order = Order(symbol="BTCUSDT", side=OrderSide.BUY, order_type=OrderType.LIMIT, quantity=0.001, price=49000.0)
...
class TestBinanceIntegration:
    ...
    async def test_full_paper_trading_workflow(self):
        ...
        with patch("src.trading.broker.binance_broker.Client") as mock_client:
            ...
            # Place sell order
            sell_order = Order(symbol="BTCUSDT", side=OrderSide.SELL, order_type=OrderType.MARKET, quantity=0.001)
