...
from src.trading.risk.post_trade import pnl_attribution, risk_reporting, trade_analysis
from src.trading.risk.pre_trade import correlation_check, exposure_limits, position_sizing
from src.trading.risk.real_time import drawdown_control, stop_loss_manager, volatility_scaling
...
class RiskController:
    ...
    def pre_trade_checks(
        ...
        within_position_limits = exposure_limits.check_position_limit(
            current_position=size, max_position=self.config.get("max_position", 1e6)
        )

        within_portfolio_limits = exposure_limits.check_portfolio_limit(
            current_exposures=current_exposures, max_portfolio_exposure=self.config.get("max_portfolio_exposure", 1e7)
        )

        correlation_ok = True
        if correlation_matrix is not None:
            correlation_ok = correlation_check.check_correlation_limit(
                correlation_matrix=correlation_matrix, threshold=self.config.get("max_correlation", 0.9)
            )
    ...
    def real_time_adjustments(
        ...
        new_stop = stop_loss_manager.dynamic_stop_loss(
            entry_price=entry_price, current_price=current_price, initial_stop=initial_stop, trailing_pct=trailing_pct
        )

        # Prefer the caller-supplied live equity over the stale config value.
        effective_equity = account_equity if account_equity is not None else self.config.get("account_equity", 100000)
        volatility_size = volatility_scaling.volatility_scaled_position(
            account_equity=effective_equity, target_vol=self.config.get("target_volatility", 0.15), returns=returns
        )

        return {"new_stop": new_stop, "volatility_size": volatility_size}
