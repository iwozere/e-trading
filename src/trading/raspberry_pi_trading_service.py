...
class RaspberryPiTradingService:
    ...
    async def load_config(self) -> bool:
        ...
        try:
            ...
            _logger.info(f"Loaded service configuration: {config_path}")
            _logger.info(f"Service: {self.config['system']['name']} v{self.config['system']['version']}")
    ...
    async def start_service(self) -> bool:
        ...
        try:
            ...
            _logger.info(f"🎯 Service running with {started_count} strategy instances")
    ...
    async def _monitor_system(self):
        ...
        while self.is_running:
            try:
                ...
                _logger.info(f"📊 System Status: CPU {system['cpu_percent']:.1f}% | "
                           f"Memory {system['memory_percent']:.1f}% | "
                           f"Temp {system['temperature_c']:.1f}°C | "
                           f"Strategies {status['strategies']['running']}/{status['strategies']['total']}")

                # Check for alerts
                alerts = self.config.get('global_settings', {}).get('monitoring', {}).get('alert_thresholds', {})

                if system['cpu_percent'] > alerts.get('cpu_usage', 80):
                    _logger.warning(f"⚠️  High CPU usage: {system['cpu_percent']:.1f}%")

                if system['memory_percent'] > alerts.get('memory_usage', 85):
                    _logger.warning(f"⚠️  High memory usage: {system['memory_percent']:.1f}%")

                if system['temperature_c'] > alerts.get('temperature', 70):
                    _logger.warning(f"⚠️  High temperature: {system['temperature_c']:.1f}°C")

                await asyncio.sleep(300)  # Monitor every 5 minutes

            except Exception as e:
                _logger.error(f"Error in system monitoring: {e}")
    ...
    async def run_forever(self):
        ...
        try:
            ...
            _logger.error(f"Service error: {e}")
            return False
        finally:
            await self.stop_service()
...
async def main():
    ...
    try:
        ...
        _logger.error(f"Service failed: {e}")
        await service.stop_service()
        sys.exit(1)
