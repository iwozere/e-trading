...
async def get_telegram_users(
    status: str | None = Query(None, description="Filter users by status: all, verified, approved, pending"),
    ...
    try:
        ...
        result_users = [TelegramUser(**user_data) for user_data in paginated_users]
...
async def verify_telegram_user(user_id: str, current_user: User = Depends(require_admin)):
...
async def approve_telegram_user(user_id: str, current_user: User = Depends(require_admin)):
...
async def reset_telegram_user_email(user_id: str, current_user: User = Depends(require_admin)):
...
async def get_telegram_user_stats(current_user: User = Depends(get_current_user)):
...
async def get_telegram_alerts(
    status: str | None = Query(None, description="Filter alerts by status: all, active, inactive"),
    page: int = Query(1, ge=1, description="Page number"),
    page_size: int = Query(50, ge=1, le=100, description="Items per page"),
    ...
    try:
        ...
        # Convert to response model
        result_alerts = [TelegramAlert(**alert_data) for alert_data in alerts_data]
...
async def toggle_telegram_alert(alert_id: int, current_user: User = Depends(require_admin)):
...
async def delete_telegram_alert(alert_id: int, current_user: User = Depends(require_admin)):
...
@router.post("/alerts")
async def create_telegram_alert(alert: CreateAlertRequest, current_user: User = Depends(require_admin)):
...
async def get_telegram_alert_stats(current_user: User = Depends(get_current_user)):
...
async def get_telegram_schedules(
    status: str | None = Query(None, description="Filter schedules by status: all, active, inactive"),
    page: int = Query(1, ge=1, description="Page number"),
    page_size: int = Query(50, ge=1, le=100, description="Items per page"),
    current_user: User = Depends(get_current_user),
):
    ...
    try:
        ...
        # Convert to response model
        result_schedules = [TelegramSchedule(**schedule_data) for schedule_data in paginated_schedules]
        ...
        _logger.info(
            "Retrieved %d Telegram schedules (page %d of %d total)", len(result_schedules), page, len(schedules_data)
        )
        return result
...
async def create_telegram_schedule(schedule: CreateScheduleRequest, current_user: User = Depends(require_admin)):
...
async def toggle_telegram_schedule(schedule_id: int, current_user: User = Depends(require_admin)):
...
async def delete_telegram_schedule(schedule_id: int, current_user: User = Depends(require_admin)):
...
async def get_telegram_schedule_stats(current_user: User = Depends(get_current_user)):
...
async def send_telegram_broadcast(broadcast: BroadcastMessage, current_user: User = Depends(require_admin)):
...
async def get_telegram_audit_logs(
    ...
    try:
        ...
        logs_data = telegram_app_service.get_audit_logs(
            limit=page_size, offset=offset, user_id=user_id, command=command, success_only=success_only
        )
...
async def get_telegram_audit_stats(current_user: User = Depends(get_current_user)):
