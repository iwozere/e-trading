...
async def get_trading_bot(bot_id: int, current_user: User = Depends(get_current_user)):
    ...
    try:
        ...
        _logger.exception(f"Error getting trading bot {bot_id}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def create_trading_bot(
    ...
    try:
        ...
        _logger.info(f"Created trading bot: {config_dict.get('name')} for user {user_id}")
...
async def update_trading_bot(
    ...
    try:
        ...
        _logger.info(f"Updated trading bot {bot_id} for user {user_id}")
        ...
        _logger.exception(f"Error updating trading bot {bot_id}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def update_bot_status(
    ...
    try:
        ...
        if success:
            _logger.info(f"Bot {bot_id} status updated to {action} by user {user_id}")
            return BotResponse(
                success=True,
        ...
        _logger.exception(f"Error updating bot {bot_id} status")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def delete_trading_bot(bot_id: int, current_user: User = Depends(get_current_user)):
    ...
    try:
        ...
        if success:
            _logger.info(f"Bot {bot_id} deleted by user {user_id}")
            return BotResponse(
                success=True,
        ...
        _logger.exception(f"Error deleting bot {bot_id}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
