...
mock_admin_user = User(id=1, email="admin@test.com", role="admin", is_active=True)

mock_regular_user = User(id=2, email="user@test.com", role="trader", is_active=True)
...
class TestTelegramBroadcast:
    ...
    def test_send_telegram_broadcast_success(self, mock_auth, mock_service):
        ...
        # Make request
        response = client.post("/api/telegram/broadcast", json={"message": "Test broadcast message"})
...
class TestTelegramAudit:
    ...
    def test_get_user_audit_logs(self, mock_auth, mock_service):
        ...
        # Mock database response
        mock_logs = [{"id": 1, "command": "/start", "success": True, "created": "2024-01-01T00:00:00Z"}]
