...
def load_performance_comparison(
    csv_path: str = "results/walk_forward_reports/performance_comparison.csv",
) -> pd.DataFrame:
...
def select_top_strategies(df: pd.DataFrame, n: int = 5) -> dict:
    ...
    for idx, row in selections["top_is_profit"].iterrows():
        ...
        _logger.info(
            "    IS Profit: $%.2f | OOS Profit: $%.2f | Robustness: %.3f",
            row["is_total_profit"],
            row["oos_total_profit"],
        ...
        for idx, row in df.iterrows():
            ...
            try:
                ...
                if os.path.exists(plot_file):
                    new_plot_path = os.path.join(category_dir, f"{strategy_id}_{is_period}.png")
