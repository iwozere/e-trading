...
from src.backtester.tests.backtester_test_framework import BacktesterTestFramework, run_backtest_from_config
...
class TestCustomStrategyBacktest:
    ...
    @pytest.mark.skipif(
        not (Path(project_root) / "data" / "BTCUSDT_1h.csv").exists(), reason="Test data file not found"
    )
...
@pytest.mark.skipif(not (Path(project_root) / "data" / "BTCUSDT_1h.csv").exists(), reason="Test data file not found")
