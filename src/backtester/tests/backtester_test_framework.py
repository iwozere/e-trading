...
class BacktesterTestFramework:
    ...
    def run_backtest(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if hasattr(strategy.analyzers, "trades"):
                ...
                results["win_rate"] = (
                    results["won_trades"] / results["total_trades"] if results["total_trades"] > 0 else 0
                )

                # Average PnL per trade
                won_total = trade_analysis.get("won", {}).get("pnl", {}).get("total", 0)
                lost_total = trade_analysis.get("lost", {}).get("pnl", {}).get("total", 0)
                results["avg_win"] = won_total / results["won_trades"] if results["won_trades"] > 0 else 0
                results["avg_loss"] = lost_total / results["lost_trades"] if results["lost_trades"] > 0 else 0
            ...
            _logger.info("Final value: %.2f, Total return: %.2f%%", final_value, results["total_return"] * 100)
    ...
    def validate_assertions(self) -> Dict[str, Any]:
        ...
        validation_results = {"passed": True, "failures": [], "checks": []}
        ...
        try:
            ...
            if "min_trades" in assertions and assertions["min_trades"] is not None:
                ...
                validation_results["checks"].append(
                    {"name": "min_trades", "passed": passed, "expected": f">= {min_trades}", "actual": actual_trades}
                )

                if not passed:
                    validation_results["passed"] = False
                    validation_results["failures"].append(f"Minimum trades not met: {actual_trades} < {min_trades}")
            ...
            if "max_drawdown_pct" in assertions and assertions["max_drawdown_pct"] is not None:
                ...
                if not passed:
                    validation_results["passed"] = False
                    validation_results["failures"].append(f"Max drawdown exceeded: {actual_dd:.2f}% > {max_dd_pct}%")
            ...
            return {"passed": False, "failures": [f"Validation error: {str(e)}"], "checks": []}
        ...
        if hasattr(self, "_validation_results"):
            ...
            if validation["checks"]:
                ...
                for check in validation["checks"]:
                    ...
                    report_lines.append(
                        f"    {status} {check['name']}: Expected {check['expected']}, Got {check['actual']}"
                    )
    ...
    def run_full_test(self, generate_report_file: bool = True) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"results": results, "validation": validation, "report": report, "success": validation["passed"]}
