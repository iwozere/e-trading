...
def optimize_cnn_lstm(trial, X_train, y_train, X_val, y_val, device):
    ...
    model = HybridCNNLSTM(
        Config.time_steps, X_train.shape[2], params["conv_filters"], params["lstm_units"], params["dense_units"]
    )
    model = train_cnn_lstm(model, X_train, y_train, device, Config.cnn_lstm_epochs, params["batch_size"], params["lr"])
    model.eval()
    with torch.no_grad():
...
def main():
    ...
    study_cnn.optimize(
        lambda trial: optimize_cnn_lstm(trial, X_train, y_train, X_test, y_test, device), n_trials=Config.n_trials
    )
    ...
    study_xgb = optuna.create_study(
        direction="minimize", study_name="xgb_opt", storage=Config.storage_name, load_if_exists=True
    )
    study_xgb.optimize(
        lambda trial: optimize_xgb(trial, train_features, y_train, test_features, y_test), n_trials=Config.n_trials
    )
