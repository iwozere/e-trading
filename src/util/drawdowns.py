...
def print_results(results):
    ...
    if results["drawdown_events"]:
        ...
        print(
            f"{'#':<3} {'Start Date':<12} {'End Date':<12} {'Start Price':<12} {'Min Price':<12} {'Peak Price':<12} {'Price Delta':<12} {'Max Drawdown':<15} {'Days':<8}"
        )
        print("-" * 115)
        ...
        for i, event in enumerate(results["drawdown_events"], 1):
            ...
            end_date = (
                event.get("end_date", "Ongoing").strftime("%Y-%m-%d")
                if hasattr(event.get("end_date"), "strftime")
                else "Ongoing"
            )
            ...
            print(
                f"{i:<3} {start_date:<12} {end_date:<12} {start_price:<12} {min_price:<12} {peak_price:<12} {price_delta:<12} {max_dd:<15} {duration:<8}"
            )
...
def plot_drawdowns(results):
    ...
    ax1.plot(results["rolling_max"].index, results["rolling_max"], label="Rolling Maximum", linewidth=1, alpha=0.7)
    ...
    # Drawdowns chart
    ax2.fill_between(results["drawdowns"].index, results["drawdowns"] * 100, 0, alpha=0.3, color="red")
    ax2.axhline(
        y=-results["threshold_percent"],
        color="red",
        linestyle="--",
