...
class Reporter:
    ...
    def extract_trade_details(self, pf: vbt.Portfolio, params: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        for idx, trade_row in records_df.iterrows():
            trade_detail = {
                ...
                "entry_time": str(trade_row["Entry Timestamp"])
                if pd.notna(trade_row.get("Entry Timestamp"))
                else str(trade_row["Entry Index"]),
                "exit_time": str(trade_row["Exit Timestamp"])
                if pd.notna(trade_row.get("Exit Timestamp"))
                else str(trade_row["Exit Index"]),
    ...
    def save_json_report(
        ...
        report_data = {
            ...
            "total_profit_after_commission": float(pf.total_profit().sum())
            if isinstance(pf.total_profit(), pd.Series)
            else float(pf.total_profit()),
            "metrics": metrics,
    ...
    def generate_html_dashboard(
        self, report_dir: str, trial_id: int, pf: vbt.Portfolio, benchmark_close: pd.Series | None = None
    ):
        """
        ...
        try:
            ...
            if isinstance(margin_usage, pd.DataFrame):
                for col in margin_usage.columns:
                    fig.add_trace(
                        go.Scatter(x=margin_usage.index, y=margin_usage[col], name=f"Margin {col}"), row=1, col=1
                    )
            else:
