...
class EODHDDataDownloader(BaseDataDownloader):
    ...
    def get_supported_intervals(self) -> List[str]:
        ...
        Note: EODHD specializes in options data, not interval-based OHLCV data.
        """
    ...
    def get_ohlcv(
        ...
        **kwargs
        ...
        Note: EODHD doesn't provide traditional OHLCV data. This method
        returns an empty DataFrame as EODHD focuses on options data.

        Args:
            symbol: Trading symbol (e.g., 'AAPL')
            interval: Data interval (not used for EODHD)
            start_date: Start date for data retrieval
            end_date: End date for data retrieval
            **kwargs: Additional provider-specific parameters

        Returns:
            Empty DataFrame (EODHD doesn't provide OHLCV data)
        """
        _logger.warning(
            "EODHD doesn't provide OHLCV data. Use fetch_chain() or "
            "download_for_date() for EODHD-specific options data."
        )
        return pd.DataFrame()
    ...
    def fetch_chain(self, ticker: str, date: Union[datetime, str]) -> pd.DataFrame:
        """
        ...
        Example:
            ...
            >>> df = downloader.fetch_chain('AAPL', '2023-12-05')
            >>> df.head()
        """
        try:
    ...
    def download_for_date(self, tickers: List[str], date: Union[datetime, str]) -> pd.DataFrame:
        ...
        Example:
            >>> downloader = EODHDDataDownloader()
            >>> df = downloader.download_for_date(['AAPL', 'MSFT'], '2023-12-05')
        """
        if not tickers:
    ...
    def compute_30d_stats(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        ...
        Example:
            >>> downloader = EODHDDataDownloader()
            >>> stats = downloader.compute_30d_stats(options_df)
        """
        try:
            if df.empty:
    ...
    def compute_uoa_score(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        Example:
            >>> downloader = EODHDDataDownloader()
            >>> uoa_scores = downloader.compute_uoa_score(stats_df)
        """
        try:
            if df.empty:
                return pd.DataFrame()
