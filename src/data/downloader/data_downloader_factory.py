...
def _build_registry() -> List[ProviderSpec]:
    ...
    specs = [
        ProviderSpec(
            ...
            "Yahoo Finance",
            YahooDataDownloader,
            aliases=["yf", "yf_finance", "yfinance"],
        ...
        ProviderSpec(
            ...
            aliases=["av", "alphavantage"],
            requires_api_key=True,
        ),
        ProviderSpec("finnhub", "Finnhub", FinnhubDataDownloader, aliases=["fh"], requires_api_key=True),
        ProviderSpec(
            "polygon", "Polygon.io", PolygonDataDownloader, aliases=["pg", "polygon_io"], requires_api_key=True
        ),
        ProviderSpec(
            "twelvedata", "Twelve Data", TwelveDataDataDownloader, aliases=["td", "twelve_data"], requires_api_key=True
        ),
        ProviderSpec("binance", "Binance", BinanceDataDownloader, aliases=["bnc"], requires_api_key=True),
        ProviderSpec(
            "coingecko", "CoinGecko", CoinGeckoDataDownloader, aliases=["cg", "coin_gecko"], requires_api_key=False
        ),
        ProviderSpec(
            "fmp",
        ...
        ProviderSpec("tiingo", "Tiingo", TiingoDataDownloader, aliases=[], requires_api_key=True),
        ProviderSpec("alpaca", "Alpaca Markets", AlpacaDataDownloader, aliases=["alp"], requires_api_key=True),
        ProviderSpec("finra", "FINRA", FinraDataDownloader, aliases=["finra_trf"], requires_api_key=True),
        ProviderSpec("eodhd", "EODHD", EODHDDataDownloader, aliases=["eod"], requires_api_key=True),
        ProviderSpec("tradier", "Tradier", TradierDataDownloader, aliases=["trdr"], requires_api_key=True),
        ProviderSpec("vix", "VIX", VIXDataDownloader, aliases=[], requires_api_key=False),
        ProviderSpec(
            "santiment", "Santiment", SantimentDataDownloader, aliases=["san", "santiment_net"], requires_api_key=False
        ),
    ]
...
class DataDownloaderFactory:
    ...
    @staticmethod
    def create_downloader(provider_code: str, **kwargs) -> BaseDataDownloader | None:
        """
    ...
    @staticmethod
    def get_provider_by_code(provider_code: str) -> str | None:
