...
class DataAggregator:
    ...
    def __init__(self, primary_provider: str = "binance"):
        ...
        _logger.info(f"Data aggregator initialized with primary provider: {primary_provider}")
    ...
    def aggregate_data(
        ...
        try:
            ...
            for provider in providers:
                ...
                if data_source:
                    ...
                    if data is not None and not data.empty:
                        ...
                        _logger.info(f"Retrieved {len(data)} data points from {provider}")
                    else:
                        _logger.warning(f"No data received from {provider}")
                else:
                    _logger.error(f"Failed to create data source for {provider}")
            ...
            if strategy == "primary_preferred":
                ...
                _logger.error(f"Unknown aggregation strategy: {strategy}")
                return None

        except Exception as e:
            _logger.error(f"Failed to aggregate data for {symbol}: {e}")
            return None
    ...
    def _aggregate_consensus(
        ...
        if not consensus_data:
            _logger.warning(f"No consensus data points for {symbol}")
            return pd.DataFrame()
    ...
    def _aggregate_best_quality(
        ...
        for provider, data in provider_data.items():
            try:
                ...
                _logger.warning(f"Failed to assess quality for {provider}: {e}")
    ...
    def _fill_data_gaps(
        ...
        _logger.info(f"Filled {len(missing_data)} gaps in {symbol} data")
    ...
    def _calculate_consensus_point(
        ...
        try:
            ...
            _logger.warning(f"Failed to calculate consensus for {timestamp}: {e}")
            return None
    ...
    def synchronize_data(
        ...
        if not common_timestamps:
            _logger.warning(f"No common timestamps found for {symbol}")
            return data1, data2
        ...
        _logger.info(f"Synchronized {len(sync_df1)} data points for {symbol}")
    ...
    def compare_data_sources(
        ...
        for provider in providers:
            try:
                ...
                _logger.error(f"Failed to analyze {provider}: {e}")
                comparison['data_points'][provider] = 0
                comparison['quality_scores'][provider] = {'quality_score': 0.0}
    ...
    def _calculate_consistency_metrics(
        ...
        try:
            ...
            _logger.error(f"Failed to calculate consistency metrics: {e}")
            metrics['error'] = str(e)
