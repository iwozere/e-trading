...
class NotificationService:
    ...
    def update_message_status(self, message_id: int, status: str, error_message: Optional[str] = None) -> Optional[Message]:
        ...
        try:
            ...
            self.repo.rollback()
            _logger.error("Failed to update message %s status: %s", message_id, e)
            raise
    ...
    def update_delivery_status(
        ...
        try:
            ...
            self.repo.rollback()
            _logger.error("Failed to update delivery status %s: %s", delivery_id, e)
            raise
    ...
    def update_channel_health(self, channel: str, status: str, error_message: Optional[str] = None) -> ChannelHealth:
        ...
        try:
            ...
            self.repo.rollback()
            _logger.error("Failed to update channel health for %s: %s", channel, e)
            raise
    ...
    def cleanup_old_messages(self, days_to_keep: int = 30) -> int:
        ...
        try:
            ...
            self.repo.rollback()
            _logger.error("Failed to cleanup old messages: %s", e)
            raise
