...
class UsersRepo:
    ...
    def get_user_by_telegram_id(self, telegram_user_id: str | int) -> User | None:
        q = (
            select(User)
            .join(AuthIdentity, AuthIdentity.user_id == User.id)
            .where(AuthIdentity.provider == PROVIDER_TG, AuthIdentity.external_id == str(telegram_user_id))
        )
        return self.s.execute(q).scalar_one_or_none()
    ...
    def get_user_notification_channels(self, user_id: int) -> Dict[str, str] | None:
        ...
        q = (
            select(User, AuthIdentity)
            .outerjoin(AuthIdentity, (AuthIdentity.user_id == User.id) & (AuthIdentity.provider == PROVIDER_TG))
        ...
        result = {"email": user.email, "telegram_chat_id": telegram_identity.external_id if telegram_identity else None}
