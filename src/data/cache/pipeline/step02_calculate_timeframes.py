...
class TimeframeCalculator:
    ...
    def load_1m_data(self, ticker: str) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            _logger.error("Error loading 1m data for %s: %s", ticker, e)
            return None
    ...
    def save_timeframe_data(self, ticker: str, timeframe: str, df: pd.DataFrame) -> bool:
        ...
        try:
            ...
            _logger.error("Error saving %s %s data: %s", ticker, timeframe, e)
            return False
        ...
        try:
            ...
            for timeframe in timeframes:
                try:
                    ...
                    _logger.error("Error processing %s %s: %s", ticker, timeframe, e)
...
Output Structure:
  DATA_CACHE_DIR/ohlcv/TICKER/TIMEFRAME/YYYY.csv.gz
    ...
    try:
        ...
        _logger.error("Pipeline Step 2 fatal error: %s", e)
        sys.exit(1)
