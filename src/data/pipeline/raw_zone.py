"""
Raw-zone writer — generalized single write path for landing external payloads.

Promoted from ``src/ml/pipeline/p22_biotech_ma/ingest/raw_zone.py`` (P22's
implementation, spec §1/§7.3) so any pipeline can reuse the same pattern
instead of each inventing its own on-disk layout:
...
This module has no pipeline-specific default: callers pass their own `root`
(typically `DATA_CACHE_DIR/<pipeline>/raw`), falling back to
`DEFAULT_RAW_ZONE_ROOT` (`DATA_CACHE_DIR/raw`) when omitted, for pipelines
that don't need their own sub-namespace. P22 continues to use its own root
(`DATA_CACHE_DIR/p22/raw`) via the backward-compatible shim left at its old
import path — see `src/ml/pipeline/p22_biotech_ma/ingest/raw_zone.py`.
"""
...
DEFAULT_RAW_ZONE_ROOT = Path(DATA_CACHE_DIR) / "raw"
...
def write(
    ...
    *,
    ...
    Args:
        ...
        root: Raw-zone root for this pipeline. Defaults to `DEFAULT_RAW_ZONE_ROOT`
            (`DATA_CACHE_DIR/raw`) — pass a pipeline-specific root (and used by tests).

    Returns:
        RawZoneWriteResult with the path, hash, known_from timestamp, and whether this
        was a new write or a dedup no-op.
    """
    zone_root = root if root is not None else DEFAULT_RAW_ZONE_ROOT
...
def read_latest_partition(source: str, *, root: Path | None = None) -> list[Any]:
    ...
    Each file in that partition is one entity's landed payload — callers get
    back the list of payloads, not a flattened/deduped row set, since what
    "dedup" means is source-specific.
...
def read_partition_before(source: str, before_date: date, *, root: Path | None = None) -> list[Any]:
    ...
    comparison pass today's date as `before_date` to see only prior runs.
...
def read_latest_partition_with_manifest(source: str, *, root: Path | None = None) -> "list[tuple[Any, dict[str, Any]]]":
    ...
    `known_from`) instead of discarding it. `known_from` matters most to
    callers: when *we* learned that payload's contents, per the bitemporal
    requirement that any downstream write derived from a landed payload use
    that timestamp, never "now" (the read/review time).
