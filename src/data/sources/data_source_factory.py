...
class DataSourceFactory:
    ...
    def _load_config(self) -> Dict[str, Any]:
        ...
        try:
            if self.config_path.exists():
                ...
                _logger.info(f"Loaded configuration from {self.config_path}")
                return config or {}
            else:
                _logger.warning(f"Configuration file not found: {self.config_path}")
                return {}
    ...
    def register_data_source(
        ...
        _logger.info(f"Registered data source class for {provider_name}")
        ...
        **kwargs
        ...
        try:
            ...
            if provider_name not in self._data_source_classes:
                _logger.error(f"No data source class registered for {provider_name}")
                return None
            ...
            _logger.info(f"Created data source for {provider_name}")
            return data_source

        except Exception as e:
            _logger.error(f"Failed to create data source for {provider_name}: {e}")
            return None
            ...
            try:
                ...
                for symbol in symbols:
                    try:
                        ...
                        _logger.warning(f"Failed to get quality report for {symbol} from {provider_name}: {e}")
                        provider_reports[symbol] = {'error': str(e)}

                quality_reports[provider_name] = provider_reports

            except Exception as e:
                _logger.error(f"Failed to get quality reports from {provider_name}: {e}")
    ...
    def cleanup_all(self) -> None:
        ...
        for provider_name, data_source in list(self._data_sources.items()):
            try:
                ...
                _logger.info(f"Cleaned up data source for {provider_name}")
            except Exception as e:
                _logger.error(f"Error cleaning up data source for {provider_name}: {e}")
    ...
    def update_provider_config(
        ...
        try:
            ...
            _logger.info(f"Updated configuration for {provider_name}")
            return True

        except Exception as e:
            _logger.error(f"Failed to update configuration for {provider_name}: {e}")
            return False
