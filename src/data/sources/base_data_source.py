...
class BaseDataSource(ABC):
    ...
    def __init__(
        ...
        _logger.info(f"Initialized {provider_name} data source")
    ...
    def get_data_with_cache(
        ...
        if cached_data is not None:
            _logger.info(f"Retrieved cached data for {symbol} {interval}")
            return cached_data
        ...
        try:
            ...
            if self.rate_limiter:
                if not self.rate_limiter.acquire():
                    _logger.warning(f"Rate limit exceeded for {self.provider_name}")
                    return None
            ...
            if data is None or data.empty:
                _logger.warning(f"No data received for {symbol} {interval}")
                return None
            ...
            if self.validation_enabled:
                ...
                if not validation_result['is_valid']:
                    _logger.warning(f"Data validation failed for {symbol}: {validation_result['errors']}")
            ...
            _logger.info(f"Successfully fetched {len(data)} data points for {symbol} {interval}")
            return data
    ...
    def _handle_error(self, message: str, error: Exception) -> None:
        ...
        _logger.error(f"{message}: {error}")

        # Log additional context for debugging
        if hasattr(error, 'response'):
            _logger.error(f"Response status: {getattr(error.response, 'status_code', 'N/A')}")
            _logger.error(f"Response text: {getattr(error.response, 'text', 'N/A')}")
    ...
    def reset_error_count(self) -> None:
        ...
        self._error_count = 0
        self._last_error = None
        _logger.info(f"Reset error count for {self.provider_name}")
    ...
    def cleanup(self) -> None:
        ...
        try:
            ...
            self._is_connected = False
            _logger.info(f"Cleaned up {self.provider_name} data source")

        except Exception as e:
            _logger.error(f"Error during cleanup of {self.provider_name}: {e}")
