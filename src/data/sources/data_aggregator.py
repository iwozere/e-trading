...
class DataAggregator:
    ...
    def aggregate_data(
        ...
        try:
            ...
            for provider in providers:
                ...
                if data_source:
                    data = data_source.get_data_with_cache(symbol, interval, start_date, end_date)
    ...
    def _aggregate_primary_preferred(
        self, provider_data: Dict[str, pd.DataFrame], symbol: str, interval: str
    ) -> pd.DataFrame:
        """
    ...
    def _aggregate_consensus(self, provider_data: Dict[str, pd.DataFrame], symbol: str, interval: str) -> pd.DataFrame:
        """
        ...
        for ts in timestamps:
            consensus_point = self._calculate_consensus_point(filtered_data, ts, symbol)
    ...
    def _aggregate_best_quality(
        ...
        sorted_providers = sorted(quality_scores.items(), key=lambda x: x[1], reverse=True)
    ...
    def _fill_data_gaps(self, base_data: pd.DataFrame, fill_data: pd.DataFrame, symbol: str) -> pd.DataFrame:
        """
    ...
    def _calculate_consensus_point(
        self, provider_data: Dict[str, pd.DataFrame], timestamp: datetime, symbol: str
    ) -> Dict[str, Any] | None:
        """
    ...
    def synchronize_data(
        self, data1: pd.DataFrame, data2: pd.DataFrame, symbol: str, tolerance: timedelta = timedelta(seconds=1)
    ...
    def compare_data_sources(
        ...
        if len(provider_data) > 1:
            comparison["consistency_metrics"] = self._calculate_consistency_metrics(provider_data, symbol)
    ...
    def _calculate_consistency_metrics(self, provider_data: Dict[str, pd.DataFrame], symbol: str) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            all_timestamps = set.intersection(*[set(data["timestamp"]) for data in provider_data.values()])
    ...
    def _generate_recommendations(self, comparison: Dict[str, Any]) -> List[str]:
        ...
        available_providers = [p for p, count in comparison["data_points"].items() if count > 0]
        ...
        if best_provider[1]["quality_score"] < 80:
            recommendations.append(
                f"Data quality is low across providers. Best: {best_provider[0]} ({best_provider[1]['quality_score']:.1f})"
            )
