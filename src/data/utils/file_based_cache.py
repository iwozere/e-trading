...
class SafeCSVAppender:
    ...
    @staticmethod
    def append_to_csv(file_path: Path, new_data: pd.DataFrame, backup_existing: bool = True) -> bool:
        """
...
class SmartDataAppender:
    ...
    @staticmethod
    def append_new_data(file_path: Path, new_data: pd.DataFrame, metadata: "CacheMetadata") -> tuple[bool, int]:
        """
        ...
        try:
            ...
            combined_data = combined_data.sort_values("timestamp").drop_duplicates(subset=["timestamp"], keep="last")
...
class CacheMetadata:
    ...
    def validate_integrity(self, file_path: Path) -> bool:
        ...
        try:
            ...
            if not df.empty:
                if (
                    df["timestamp"].min().isoformat() != self.first_timestamp
                    or df["timestamp"].max().isoformat() != self.last_timestamp
                ):
                    return False
...
class FileCacheCompressor:
    ...
    def save_compressed_csv(self, df: pd.DataFrame, file_path: Path, compression: str = "gzip") -> bool:
    ...
    def load_compressed_csv(self, file_path: Path) -> pd.DataFrame | None:
...
class FileBasedCache:
    ...
    def __init__(
        ...
        invalidation_strategies: List[FileCacheInvalidationStrategy] | None = None,
    ...
    def _get_data_path(self, cache_path: Path, format: str = "csv", year: int | None = None) -> Path:
        """Get data file path."""
        if format == "parquet":
            if year:
    ...
    def _load_metadata(self, cache_path: Path) -> Dict[str, Any] | None:
    ...
    def _split_data_by_years(
        self, df: pd.DataFrame, start_date: datetime | None = None, end_date: datetime | None = None
    ...
    def _get_years_to_check(
        ...
        if start_date and end_date:
            ...
            _logger.debug(
                "Both dates specified: start_year=%s (%s), end_year=%s (%s)",
                start_year,
                type(start_year),
                end_year,
                type(end_year),
            ...
            current_year = datetime.now().year
            _logger.debug(
                "Only start date: start_year=%s (%s), current_year=%s (%s)",
                start_year,
                type(start_year),
                current_year,
                type(current_year),
            ...
            _logger.debug(
                "Only end date: start_year=%s (%s), end_year=%s (%s)",
                start_year,
                type(start_year),
                end_year,
                type(end_year),
                        ...
                        for interval_dir in symbol_dir.iterdir():
                            ...
                            for file_path in interval_dir.iterdir():
                                if file_path.is_file():
                                    ...
                                    _logger.debug(
                                        "Checking file: %s, stem: %s, isdigit: %s",
                                        file_path,
                                        filename,
                                        filename.isdigit(),
                                    )
    ...
    def migrate_existing_data(self) -> Dict[str, Any]:
        ...
        try:
                                ...
                                if len(years_in_data) > 1:
                                    ...
                                    if success:
                                        ...
                                        migration_results["details"].append(
                                            f"{provider}/{symbol}/{interval}/{year} -> {years_in_data}"
                                        )
                                    else:
            ...
            _logger.info(
                "Cache migration completed: %d files migrated, %d years created",
                migration_results["files_migrated"],
                migration_results["years_created"],
            )
    ...
    def get(
        ...
        try:
            with self.lock:
                ...
                _logger.debug(
                    "Cache hit for %s/%s/%s across %d years: %d total rows",
                    provider,
                    symbol,
                    interval,
                    len(all_data),
    ...
    def put(
        ...
        try:
            with self.lock:
                ...
                for year, year_df in years_data.items():
                    ...
                    if format == "csv":
                        if append_mode and data_path.exists():
                            ...
                            success, rows_added = SmartDataAppender.append_new_data(data_path, year_df, cache_metadata)
                            if success:
                    ...
                    _logger.debug(
                        "Cached data for %s/%s/%s/%s: %d rows", provider, symbol, interval, year, len(year_df)
                    )
                ...
                _logger.info(
                    "Successfully cached %d total rows across %d year files for %s/%s/%s",
                    total_rows_cached,
                    files_created,
                    provider,
                    symbol,
    ...
    def clear(
        self, provider: str | None = None, symbol: str | None = None, interval: str | None = None
    ) -> bool:
        """
...
_file_cache_instance: FileBasedCache | None = None
