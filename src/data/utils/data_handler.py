...
class DataHandler:
    ...
    def standardize_ohlcv_data(
        ...
        if len(df_std) < initial_rows:
            _logger.warning(f"Removed {initial_rows - len(df_std)} rows with invalid data for {symbol}")
    ...
    def validate_and_score_data(
        ...
        # Log results
        if not is_valid:
            _logger.warning(f"Data validation failed for {symbol}: {errors}")
            if strict_validation:
                raise ValueError(f"Data validation failed for {symbol}: {errors}")
        else:
            _logger.info(f"Data validation passed for {symbol}")

        _logger.info(f"Data quality score for {symbol}: {quality_score['quality_score']:.2f}")
    ...
    def cache_data(
        ...
        try:
            ...
            _logger.error(f"Failed to cache data for {symbol}: {e}")
            return False
    ...
    def get_cached_data(
        ...
        try:
            ...
            _logger.error(f"Failed to retrieve cached data for {symbol}: {e}")
            return None
    ...
    def save_data(
        ...
        try:
            ...
            _logger.error(f"Failed to save data to {filepath}: {e}")
            raise
    ...
    def load_data(self, filepath: Path) -> pd.DataFrame:
        ...
        try:
            ...
            _logger.info(f"Loaded data from {filepath}")
            return df

        except Exception as e:
            _logger.error(f"Failed to load data from {filepath}: {e}")
            raise
    ...
    def merge_data(
        ...
        if len(combined) < initial_rows:
            _logger.info(f"Removed {initial_rows - len(combined)} duplicate rows for {symbol}")
    ...
    def resample_data(
        ...
        try:
            ...
            _logger.info(f"Resampled {symbol} data to {target_interval} interval")
            ...
            _logger.error(f"Failed to resample data for {symbol}: {e}")
            return df
