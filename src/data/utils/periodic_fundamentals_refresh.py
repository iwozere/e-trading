...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("fundamentals_refresh.log"), logging.StreamHandler()],
)
...
def main():
    ...
    symbol_group.add_argument("--symbols", type=str, help="Comma-separated list of symbols to refresh")
    symbol_group.add_argument("--all-symbols", action="store_true", help="Refresh all cached symbols")
    symbol_group.add_argument("--cleanup-only", action="store_true", help="Only cleanup expired cache data")
    ...
    parser.add_argument(
        "--data-types", type=str, default="ratios,profile", help="Data types to refresh (default: ratios,profile)"
    )
    parser.add_argument(
        "--max-symbols", type=int, default=50, help="Maximum number of symbols to process (default: 50)"
    )
    ...
    try:
        ...
        if args.cleanup_only:
            ...
            logger.info(
                "Cleanup completed: %d files, %d symbols removed",
        ...
        if args.all_symbols:
            ...
            logger.info("Found %d cached symbols, processing first %d", len(all_symbols), len(symbols))
        elif args.symbols:
