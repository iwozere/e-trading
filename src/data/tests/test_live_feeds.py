...
class TestBinanceLiveFeed(TestLiveFeeds):
    ...
    def test_interval_conversion(self, mock_klines):
        ...
        feed = BinanceLiveDataFeed(symbol="BTCUSDT", interval="1h", api_key=self.api_key, api_secret=self.api_secret)
    ...
    def test_get_status(self, mock_klines):
        ...
        feed = BinanceLiveDataFeed(
            symbol="BTCUSDT", interval=self.interval, api_key=self.api_key, api_secret=self.api_secret
        )
...
class TestYahooLiveFeed(TestLiveFeeds):
    ...
    def test_historical_data_loading(self, mock_download):
        ...
        feed = YahooLiveDataFeed(symbol="BTCUSDT", interval=self.interval, lookback_bars=self.lookback_bars)
    ...
    def test_polling_mechanism(self, mock_ticker, mock_download):
        ...
        # Mock historical data
        mock_data = pd.DataFrame(
            {"Open": [150.0], "High": [155.0], "Low": [149.0], "Close": [151.0], "Volume": [1000000]},
        ...
        mock_ticker_instance.info = {"regularMarketPrice": 150.0, "regularMarketVolume": 1000000}
        # Mock the history method to return data with a future timestamp
        mock_ticker_instance.history.return_value = pd.DataFrame(
            {"Open": [150.0], "High": [155.0], "Low": [149.0], "Close": [151.0], "Volume": [1000000]},
            index=pd.date_range("2025-09-07", periods=1, freq="D"),
        )  # Future date to ensure new data
    ...
    def test_get_status(self, mock_download):
        ...
        feed = YahooLiveDataFeed(symbol="BTCUSDT", interval=self.interval)
...
class TestIBKRLiveFeed(TestLiveFeeds):
    ...
    def test_get_status(self, mock_ib_connect, mock_req_contract, mock_req_historical, mock_connect):
        ...
        feed = IBKRLiveDataFeed(
            symbol=self.test_symbol, interval=self.interval, host=self.host, port=self.port, client_id=self.client_id
        )
...
class TestCoinGeckoLiveFeed(TestLiveFeeds):
    ...
    def test_historical_data_loading(self, mock_get):
        ...
        feed = CoinGeckoLiveDataFeed(symbol="bitcoin", interval=self.interval, lookback_bars=self.lookback_bars)
    ...
    def test_polling_mechanism(self, mock_get):
        ...
        mock_response.json.return_value = {
            "bitcoin": {"usd": 150.0, "usd_24h_vol": 24000000, "last_updated_at": 1672531200}
        }
    ...
    def test_rate_limiting(self, mock_get):
        ...
        feed = CoinGeckoLiveDataFeed(symbol="bitcoin", interval=self.interval)
...
class TestDataFeedFactory(unittest.TestCase):
    ...
    def test_create_yahoo_feed(self):
        ...
        config = {"data_source": "yahoo", "symbol": "AAPL", "interval": "1d"}
    ...
    def test_create_coingecko_feed(self):
        ...
        config = {"data_source": "coingecko", "symbol": "bitcoin", "interval": "1d"}
    ...
    def test_invalid_data_source(self):
        ...
        config = {"data_source": "invalid", "symbol": "AAPL", "interval": "1d"}
...
class TestBaseLiveDataFeed(unittest.TestCase):
    ...
    def test_common_functionality(self):
        ...
        with patch("yfinance.download") as mock_download:
            ...
            feed = YahooLiveDataFeed(symbol="BTCUSDT", interval="1d", lookback_bars=1)
