...
class TestSmartDataAppender(unittest.TestCase):
    ...
    def test_append_new_data_new_file(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            file_path = Path(temp_dir) / "test.csv"
            metadata = CacheMetadata(provider="test", symbol="TEST", interval="1h", year=2023)
...
class TestEnhancedFileBasedCache(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.cache = FileBasedCache(cache_dir=self.temp_dir, compression_enabled=False, default_format="csv")
    ...
    def test_put_with_csv_format(self):
        ...
        success = self.cache.put(self.test_df, "test_provider", "TEST", "1h", format="csv")
        self.assertTrue(success)
    ...
    def test_put_with_append_mode(self):
        ...
        # First put
        success = self.cache.put(self.test_df, "test_provider", "TEST", "1h", format="csv", append_mode=True)
        self.assertTrue(success)
        ...
        success = self.cache.put(new_data, "test_provider", "TEST", "1h", format="csv", append_mode=True)
        self.assertTrue(success)
    ...
    def test_csv_format_validation(self):
        ...
        success = self.cache.put(invalid_df, "test_provider", "TEST", "1h", format="csv")
