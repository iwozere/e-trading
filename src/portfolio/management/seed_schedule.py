"""
Seed the `job_schedules` row that drives the earnings-triggered stop-loss
coverage reminder.
...
DEFAULT_SCHEDULE_NAME = "portfolio_stop_loss_reminder"
DEFAULT_TARGET = "portfolio.management"


def _parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    """Parse CLI arguments for seeding."""
    parser = argparse.ArgumentParser(
        prog="python -m src.portfolio.management.seed_schedule",
        description="Insert or update the job_schedules row for the stop-loss coverage reminder.",
    )
    parser.add_argument(
        "--config",
