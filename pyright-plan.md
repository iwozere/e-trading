...
- 2026-07-12 — **Phase D batches D3–D8: 229 → 87 errors** (test code long tail). One imperative commit per batch; mypy verified 0 after each; no test regressions (stash-baseline compared where suites run).
  - **D3 (6f2839f) 229→202** — sentiments suite. `test_unified_sentiment`: cast `collect_sentiment_batch` result to the dataclass shape (return is a `Union[..., str]` for the json `output_format`). `test_async_trends`/`test_async_news`: dropped assertions + the proxy-only test referencing the removed proxy/config API (`proxy_list`/`trends_base`/`requests_per_hour`/`regions`/`_get_with_retry`/`*_base`). Pre-existing adapter-refactor suite-rot in `test_async_trends` is out of pyright scope.
  - **D4 (e61fd2f) 202→158** — data/db + FMP. Deleted `test_database_integration.py` (dead: generic `create`/`get_by_id`/`update`/`get_all` CRUD API never existed on Bots/Trades/MetricsRepo; permanently Postgres-gated → no live coverage). `test_service_jobs`: `job_id` as `str` (model is `str|None` + coerce validator; the "must be int" comments were wrong). `test_fmp_data_downloader`/`test_fmp_demo`: `base_url`→`stable_url`, drop removed `_validate_screener_criteria` test, rewrite fundamentals block to current dict shape (`profile`/`ratios` sub-dicts vs removed flat object). FMP mock tests pass (7).
  - **D5 (59952cb + user's 8e2b4f9) 158→137** — telegram screener. Real bug: the three enhanced-screener tests called async `run_enhanced_screener()` without awaiting, so every `report.*` access hit a coroutine (silently swallowed by try/except). Fixed via `asyncio.run(...)` (+`import asyncio`); price via `result.fundamentals.current_price` (`ScreenerResult` has no `current_price`); guard `len(df)` for `DataFrame|None`.
  - **D6 (7b8fc75) 137→109** — business-logic/service-layer. `telegram_service_mock`: `should_raise_errors` typed `Dict[str, BaseException | Callable[[], BaseException]]` + two implicit-Optional params. NewType-wrapped `TickerIndicatorsRequest` (Ticker/Indicator/TimeFrame/Period) + keyword `IndicatorValue` across `test_business_logic`/`_core`/`test_service_layer_mocks`/`test_integration_service_layer`. Deleted 2 dead tests calling never-existent `TelegramBusinessLogic.get_service_health_status` (health lives on src/api health services). Pre-existing fixture-wiring gap (fixtures not registered via conftest) unchanged.
  - **D7 (0d8538e) 109→99** — deleted `tests/test_trading_bot_integration.py`: all 3 tests fail at setup (`@patch` targets removed module attrs `EmailNotifier`/`create_telegram_notifier`; drives removed `bot.telegram_notifier`/`email_notifier`). BaseTradingBot now uses `NotificationServiceClient` + gained a required `parameters` arg. NOTE: its core behaviours (process_signals→positions/trade_history/trades.json, state persistence) are worth re-testing against the new architecture.
  - **D8 (75eb475) 99→87** — phase4 data. `test_phase4_integration`: explicit `assert x is not None` after `assertIsNotNone` (pyright doesn't narrow through unittest asserts) for `DataFrame|None` returns. `run_phase4_tests`: `str(start_dir)` for `unittest discover`; convert the "RedisCache removed" negative-import check to a runtime `importlib`/`hasattr` probe.
  Remaining 87 errors spread thin across ~40 test files (reportArgumentType 44, reportAttributeAccessIssue ~30, reportCallIssue ~22).

- 2026-07-12 — **Phase D COMPLETE: 87 → 0 errors. Entire active codebase is pyright-clean (production + tests, 0 errors).** Final batches:
  - **D9 (bb9ea4a) 87→77** — p04 short-squeeze. TransientMetrics gained required call_put_ratio/borrow_fee_pct (added None to the 4 validation tests → fixed 4 failing tests); assert-not-None before assertAlmostEqual; construct+pass the now-required structural_metrics; delete two dead UniverseLoader.filter_by_* tests (filtering is internal now).
  - **D10 (3c2add7) 77→73** — cron_parser (×2 files). getattr(tzinfo, "zone", None) (pytz-only attr); cast(str, ...) for bad-type negative tests.
  - **D11 (1b482fe) 73→51** — notification cluster. Production fix: batch_processor callback typed Callable[[MessageBatch], Awaitable[None]] (it's awaited). check_delivery_status uses MessageDeliveryStatus (DeliveryStatus is now a StrEnum); ChannelHealth status/last_check (no channel/checked_at); notification_client→notification_db_service; ParsedCommand in the ignored `parsed` slot.
  - **D12 (52f6cb3) 51→47** — data/db models. Production fix: model_trading Mapped[Numeric]→Mapped[Decimal] (×14; value type is Decimal). Drop removed BotInstance.type; JobType.REPORT; job_id=str(...); delete test for removed get_finra_data_count_for_date.
  - **D13 (3f89eaa) 47→20** — mechanical long-tail (12 files). assert-not-None after assertIsNotNone; cast for negative bad-type args; Optional test-helper params; cast(List[BaseStrategy], [Mock]); nested source_breakdown; guard `in` on str|None. Also fixed a pre-existing mypy annotation (job_snapshot: dict).
  - **D14 (bf938a7) 20→0** — final long-tail (13 files). backtrader Any-aliases (_bt.TimeFrame.Days, _PandasData(dataname=)); API drift (channel_keywords removed, _screen_ticker→_screen_ticker_individual, cast over parametrized downloader union); assert-not-None; cast for None/Mock wiring; IBKRBroker required args; talib.RSI(np.asarray(...)); DatetimeIndex(...).tz_localize.
  Production changes this phase (all mypy 0, zero pyright ripple): model_trading Mapped[Decimal], batch_processor Awaitable callback.
  Dead suites deleted across Phase D: test_client, test_new_config_system, test_bot_manager, test_batch_processing, test_database_integration, test_trading_bot_integration.

**NEXT: Phase E** — pin pyright version in requirements-dev, add CI gate (switch the ratchet to plain `npx pyright` now that count is 0), resolve the .venv-vs-system-Python-3.13 split-brain, then delete pyright-plan.md.
