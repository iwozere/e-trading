...
"""
Test Business Logic
Comprehensive unit tests for telegram business logic using service layer mocks.
Tests business logic in isolation from database and external services.
"""
...
class TestTelegramBusinessLogic:
    """Test telegram business logic methods with service layer mocks."""
    ...
    @pytest.mark.asyncio
    async def test_handle_command_dispatch(self, business_logic_with_mocks):
        ...
        # Test help command
        help_command = create_parsed_command("help", "test_user")
        result = await business_logic.handle_command(help_command)

        assert result["status"] == "ok"
        assert "help_text" in result

        # Test unknown command
        unknown_command = create_parsed_command("unknown", "test_user")
        result = await business_logic.handle_command(unknown_command)

        assert result["status"] == "error"
        assert "Unknown command" in result["message"]

    @pytest.mark.asyncio
    async def test_handle_command_service_unavailable(self, indicator_service_mock):
        """Test command handling when telegram service is unavailable."""
        # Create business logic with None telegram service
        business_logic = TelegramBusinessLogic(None, indicator_service_mock)

        command = create_parsed_command("help", "test_user")
        result = await business_logic.handle_command(command)
    ...
    @pytest.mark.asyncio
    async def test_handle_command_requires_indicator_service(self, business_logic_with_mocks):
        """Test commands that require indicator service."""
        business_logic, telegram_service_mock, _ = business_logic_with_mocks

        # Setup user
        telegram_service_mock.add_test_user("test_user", approved=True, verified=True)

        # Disable indicator service
        business_logic.indicator_service = None

        # Test report command (requires indicator service)
        report_command = create_parsed_command("report", "test_user", tickers=["AAPL"])
        result = await business_logic.handle_command(report_command)

        assert result["status"] == "error"
        assert "Indicator calculation service temporarily unavailable" in result["message"]
        assert result["error_type"] == "IndicatorServiceUnavailable"
    ...
    def test_handle_help_command(self, business_logic_with_mocks):
        ...
        help_command = create_parsed_command("help", "regular_user")
        result = business_logic.handle_help(help_command)
        ...
        admin_help_command = create_parsed_command("help", "admin_user")
        admin_result = business_logic.handle_help(admin_help_command)

        assert admin_result["status"] == "ok"
        assert admin_result["is_admin"] is True

    def test_handle_help_command_missing_user_id(self, business_logic_with_mocks):
        """Test help command with missing user ID."""
        business_logic, _, _ = business_logic_with_mocks

        help_command = ParsedCommand(command="help", args={}, positionals=[])
        result = business_logic.handle_help(help_command)

        assert result["status"] == "error"
        assert "No telegram_user_id provided" in result["message"]
    ...
    @pytest.mark.asyncio
    async def test_error_wrapper_functionality(self, business_logic_with_mocks):
        """Test the error wrapper for command handlers."""
        business_logic, telegram_service_mock, indicator_service_mock = business_logic_with_mocks
        ...
        result = await business_logic._handle_with_error_wrapper(success_handler, command)

        assert result["status"] == "ok"
        assert result["message"] == "Success"

        # Test handler that raises exception
        def error_handler(parsed):
            raise ValueError("Test error")

        error_result = await business_logic._handle_with_error_wrapper(error_handler, command)

        assert error_result["status"] == "error"
        assert "error occurred" in error_result["message"].lower()
        assert error_result["error_type"] == "ValueError"

    @pytest.mark.asyncio
    async def test_error_wrapper_requires_indicator_service(self, business_logic_with_mocks):
        """Test error wrapper when indicator service is required but unavailable."""
        business_logic, telegram_service_mock, _ = business_logic_with_mocks

        # Disable indicator service
        business_logic.indicator_service = None

        def handler_requiring_indicators(parsed):
            return {"status": "ok", "message": "Success"}
