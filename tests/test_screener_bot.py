    ...
    if email and email_body:
        ...
        async def send_email_async():
            notification_manager = await initialize_notification_manager(
                email_sender=TEST_SMTP_USER, email_receiver=email
            )
...
def test_db_user_registration():
    ...
    sent_time = datetime.now(UTC).strftime("%Y-%m-%d %H:%M:%S")
...
def test_my_register_and_verify():
    ...
    if datetime.now(UTC) > sent_dt + timedelta(hours=1):
        ...
        set_user_verified(TEST_USER_ID, datetime.now(UTC).strftime("%Y-%m-%d %H:%M:%S"))
...
def test_my_status_and_analyze():
    ...
    async def send_email_async():
        notification_manager = await initialize_notification_manager(email_sender=TEST_SMTP_USER, email_receiver=email)
        await notification_manager.send_notification(
            notification_type=NotificationType.INFO,
            title=f"Your Screener Status Report - {len(pairs)} Tickers Analyzed",
            message="<h2>Your Screener Status Report</h2><hr>" + "<br><br>".join(email_body),
