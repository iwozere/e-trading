...
class EnhancedMultiStrategyRunner:
    ...
    async def load_config(self) -> bool:
        ...
        try:
            ...
            _logger.info(f"Loaded configuration: {config_path}")
            _logger.info(f"System: {self.config['system']['name']} v{self.config['system']['version']}")
            _logger.info(f"Strategies to run: {len(self.config['strategies'])}")
    ...
    async def validate_config(self) -> bool:
        ...
        for section in required_sections:
            if section not in self.config:
                _logger.error(f"Missing required section: {section}")
                return False
        ...
        for i, strategy in enumerate(self.config['strategies']):
            ...
            for field in required_fields:
                if field not in strategy:
                    _logger.error(f"Strategy {i}: Missing required field '{field}'")
                    return False
    ...
    async def start_strategy(self, strategy_config: Dict) -> bool:
        ...
        try:
            _logger.info(f"Starting strategy: {strategy_name} ({strategy_id})")
            ...
            if not broker_id:
                _logger.error(f"Failed to create broker for strategy {strategy_name}")
                return False
            ...
            if success:
                ...
                _logger.info(f"✅ Strategy {strategy_name} started successfully")
                return True
            else:
                _logger.error(f"❌ Failed to start broker for strategy {strategy_name}")
                return False

        except Exception as e:
            _logger.error(f"Error starting strategy {strategy_name}: {e}")
            return False

    async def stop_strategy(self, strategy_id: str) -> bool:
        """Stop a single strategy."""
        if strategy_id not in self.running_strategies:
            _logger.warning(f"Strategy {strategy_id} not found in running strategies")
            return False
        ...
        try:
            _logger.info(f"Stopping strategy: {strategy_name}")
            ...
            if success:
                ...
                _logger.info(f"✅ Strategy {strategy_name} stopped successfully")
                return True
            else:
                _logger.error(f"❌ Failed to stop strategy {strategy_name}")
                return False

        except Exception as e:
            _logger.error(f"Error stopping strategy {strategy_name}: {e}")
            return False
    ...
    async def start_all_strategies(self) -> int:
        ...
        _logger.info(f"Started {started_count}/{len(self.config['strategies'])} strategies")
    ...
    async def monitor_strategies(self):
        ...
        while self.is_running:
            try:
                ...
                _logger.info(f"📊 Strategy Status Report - {len(self.running_strategies)} active")
                _logger.info("-" * 80)
                ...
                for strategy_id, strategy_info in self.running_strategies.items():
                    ...
                    if status:
                        ...
                        _logger.info(f"🤖 {strategy_name}:")
                        _logger.info(f"   Status: {status.get('status', 'Unknown')}")
                        _logger.info(f"   Health: {health}")
                        _logger.info(f"   Uptime: {uptime:.0f}s")
                        ...
                        try:
                            ...
                            if broker and hasattr(broker, 'get_portfolio'):
                                ...
                                if portfolio:
                                    ...
                                    _logger.info(f"   Portfolio: ${total_value:.2f} (P&L: ${pnl:.2f})")
                        except Exception as e:
                            _logger.debug(f"   Portfolio error: {e}")

                        # Check for unhealthy strategies
                        if health != 'healthy':
                            _logger.warning(f"⚠️  Strategy {strategy_name} health issue: {health}")
                    else:
                        _logger.warning(f"❌ {strategy_name}: No status available")

                # System summary
                uptime = (datetime.now(timezone.utc) - self.start_time).total_seconds()
                _logger.info(f"📈 System uptime: {uptime:.0f}s | Active strategies: {len(self.running_strategies)}")
    ...
    async def run(self):
        ...
        try:
            ...
            _logger.info(f"🎯 System running with {started_count} strategies")
            ...
            _logger.error(f"Error in main run loop: {e}")
            return False
        finally:
            await self.shutdown()
